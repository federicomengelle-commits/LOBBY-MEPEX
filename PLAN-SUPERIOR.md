...
> Última consolidación: **2026-07-22** (peinada post-sesiones CRM ficha v4 del 18-19/07 + verificación de prod: v40/v87 servidas, PostHog v2, `wa_eventos` ya en prod, webhook WA aún sin deploy en VPS).
...
1. **WhatsApp E4 — sesión "con celu"** 📅 **agendada: Fede trae el celu de la oficina mar→mié o mié→jue de la semana entrante.** Runbook paso a paso listo: `docs/whatsapp-coexistence-runbook.md` (SQL `whatsapp_webhook_v1.sql` → deploy webhook + `WA_VERIFY_TOKEN` → app Meta + App Secret → Business Verification con constancia AFIP → configurar webhook + subscribe 4 campos → conectar número desde el celu → smoke E2E contra `wa_eventos`). Traer: celu con Business App ≥2.24.17 + constancia AFIP + 30-45 min. **Estado verificado 2026-07-22:** SQL `wa_eventos` ✅ YA en prod; webhook todavía 404 en prod (falta el paso de deploy en VPS del runbook — se hace en la sesión); el front v40 con el composer listo para enchufar el envío real. **Después: fase 2** (procesador `wa_eventos`→timeline `crm_mensajes`, matcheo por teléfono —ojo columnas rotadas de `clientes`—, mandar desde el CRM) — se diseña juntos post-conexión.
...
4. **PostHog Analytics** ✅ **INTEGRADO 2026-07-17** (cuenta creada por Fede — US Cloud, proyecto 518740, cuenta personal federicomengelle@; key phc_ client-side en `posthog-init.js`): carga post-login al final de `App._APP_SCRIPTS` como **script OPCIONAL** (si el CDN falla, la app carga igual — `_OPTIONAL_SCRIPTS`), identify usuario+rol, $pageview manual por hashchange con propiedad `modulo`, CSP actualizada (us-assets script / us.i connect). Verificado en local con CSP enforcing: init + identify + connect 200 + 0 violaciones. **✅ DEPLOYADO Y VERIFICADO EN PROD 2026-07-17** (posthog cargado + identificado como `fede`, eventos fluyendo; onboarding "Installation complete"). Residuales finos: (a) ~~1 pull más para el **Error Tracking**~~ ✅ tomado (`posthog-init.js?v=2` verificado en prod 2026-07-22) · (b) **Session replay**: activarlo desde PostHog Settings cuando Fede quiera (el código ya lo banca) · (c) productos elegidos: Product/Web Analytics + Session Replay + Error Tracking (Logs/AI/Flags descartados para herramienta interna).
...
- ~~**🎨 CRM Bandeja — rediseño VISUAL**~~ ✅ **DISEÑADO + CONSTRUIDO 2026-07-19** (commit `5ef1f7a`, sesión en vivo con 6 mockups): Bandeja en **cards 2-col** (cliente protagonista, acciones inline, sin montos) + **Ficha de Caso v3** "sala de situación" (cabecera rica, Resumen IA estilo Gemini, cotización desplegable sin montos, Drive del caso, ruta `#crm/caso/<id>`, chip evento deep-link). Reviews de los 3 agentes nativos pasados (3 HIGH cazados y arreglados). SQL+pull+connector ✅ hechos por Fede; **resumen IA real andando en prod**. Ronda 2 de densidad/íconos pusheada (`b9f15e9`); header ✅ aprobado ("mejoró mucho"). **✅ FLUJO DEL COMPOSER CERRADO 2026-07-19** (sesión ficha v4, plan-first cumplido: diagrama + 2 mockups → Fede eligió **M2 "Copiloto"**): composer = **MANDAR** (WhatsApp `wa.me` puente + Nota, Enter manda; "Respuestas frecuentes" editables; registro manual delicado escondido en ＋) · **resumen IA automático e INCREMENTAL** ("lo que está hecho ya está hecho", mode `resumen_caso_inc`, Rehacer de cero discreto) · **Copiloto** (sugerencia + Redactar respuesta + toggle 48h sin DDL) · **countdown del evento** ya incluido · adjuntar COT PDF/Drive ya en ＋ · v4.1: pipeline y fichas **sin importes** + ficha de cliente igualita + back circular. 4 tandas pusheadas (`14d444b`→`2799707`) y **verificadas en prod (v40/v87 servidas, chequeado 2026-07-22)**. **Fase 2 (post-celu):** enchufar el envío real por Cloud API en el composer (sin rediseño — el punto de enchufe quedó hecho). **Pendiente corto conexo:** ~~barrido global botón-volver FUERA de CRM~~ ✅ hecho 2026-07-22 (ver ítem siguiente).
- ~~**🎨 Patrón global "botón volver"**~~ ✅ **BARRIDO HECHO 2026-07-22:** clase global `.btn-back-circ` en `style.css?v=18` (calcada de `.caso-back-v3` del CRM) aplicada a los 8 back de texto restantes — Finanzas (cuenta detail + conciliación), Inventario físico, Locaciones, Stands, Compras (proveedor + OC, tenía chevron+texto), importar-contactos e importar-cotización — + el ✕ del panel de Eventos alineado al mismo lenguaje (solo CSS). CSS huérfano borrado (5 clases). NO tocados a propósito: wizards "← Atrás", prev/next de catálogo, nav de calendario, breadcrumb de proyecto-detalle (otro patrón, prolijo). Reviewer typescript: Approve 0 C/H (LOW anotado: 30px < 44px táctil mobile, mismo lote que `.caso-back-v3` si se revisa). IDs/handlers intactos, verificado en preview.
...
## 🎯 Orden recomendado (actualizado 2026-07-22)
**Esta semana: WhatsApp sesión con celu (agendada, runbook + SQL ya listos) → fase 2 E4 (procesador `wa_eventos`→timeline + envío real desde el composer) → ~~barrido botón-volver global~~ ✅ hecho 22/07 → MFA Lelean/Sofi (5 min c/u).**
Después: **CSV de Meli → ronda de testeo (¿27/07 o agosto? — decidir fecha; ventaja: ya probarían con WhatsApp conectado) → triage → OCTEXA Pilar 2 en paralelo.**
Gmail sigue en la heladera hasta que iPlan destrabe. Marketing/base arranca cuando Fede junte los archivos. Claude-mem: fin del piloto ~25/07 (decidir si queda o se apaga en LOBBY).
