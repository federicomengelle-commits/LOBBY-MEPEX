...
### 🔴 Ahora (esta semana)
- **Largar la ronda de testeo:** `~/pull-lobby.sh` final → confirmar que **`/lobby-api` responde** (si no, a Lelean le falla el reset de contraseña) → mandar los WhatsApp (`docs/testeo/mensajes-whatsapp.md`) + adjuntar PDFs → crear el grupo de reporte. **Antes, una pasada logueado:** responder 1 encuesta PRUEBA, 1 cobro/pago que genere asiento, subir 1 foto de armado.
...
- **Git housekeeping:** commitear/pushear `docs/testeo/` · borrar la rama vieja `pulido-proyectos` (sus commits ya están en main con otro hash) · `importar-3dsmax.js` queda untracked a propósito.
