...
2. **Consultoría Jordi (NUEVO frente).** Seguridad ✅ COMPLETA (39/40, `docs/cierre-auditoria-jordi.md`, verificada en prod 2026-07-17; bug grave de disponibilidad cazado en el camino). **Siguiente fase:** los 4 MD de Jordi (`docs/jordi/`) + 3 plugins + PostHog + WhatsApp rápido — todo dentro de la ventana de la consultoría.
...
2. **Los 4 MD de Jordi** → Fede los deja en **`docs/jordi/`** → sesión dedicada con el prompt que está en `docs/jordi/README.md` (leer → cruzar contra estado real → veredicto por MD → accionables acá / reglas destiladas a CLAUDE.md).
3. **3 plugins (links los pasa Fede, NO custom):** oficial de **Supabase** + uno de **persistencia de memoria** (el que usa Jordi) + **Superpowers**. Instalación en sesión aparte junto con los MD (o antes — es rápido). ⚠️ Al instalar el de memoria: evaluar solape con el sistema de memoria actual (archivo `memory/`) para no duplicar. El de Supabase es ORO para este repo: acceso directo a schema/SQL sin pasar queries a mano.
4. **PostHog Analytics** (recomendación de Jordi): ⏳ **Fede se crea la cuenta** → después lo integramos (snippet en `index.html` post-login o self-host en el VPS — decidir en esa sesión; ojo CSP: sumar el host a `script-src`/`connect-src`).
5. **Confirmar triggers de notificaciones** (`sql/notif_operativas.sql`) — 1 query de 10 seg en SQL Editor: `SELECT tgname FROM pg_trigger WHERE tgname LIKE 'trg_notif%';` (esperado: `trg_notif_stock_minimo` + `trg_notif_equipo_estado`). Único SQL no verificable por REST.

### 🟠 Cierres cortos (dependen solo de vos / guiados)
- **Probar el circuito de gastos de evento en prod** (cargar comprobante→Rendimiento → cerrar → migrar/conciliar a Egresos). El SQL ya está; escribe datos reales → probalo vos o guiado con cleanup.
...
- **🎨 CRM Bandeja — rediseño VISUAL (dirección de Fede 2026-07-17):** quiere "una visualización excelente de todo" — hoy las filas son líneas horizontales largas; explorar **más onda cards (o híbrido filas/cards)**, bien visual, **hasta botoncitos inline**. Método: `pulir-pantallas` con mockups (`show_widget`) ANTES de tocar código — es subjetivo, va charla en vivo. La lógica de la Bandeja v2 (triage/snooze/chips/acciones hover) NO se toca, es re-skin de presentación.
- **Triage de la ronda** → arreglar lo que el equipo cace → pulido dirigido con criterio real.
- **Finanzas restantes:** Fase 5 conciliación bancaria CSV (Galicia/MercadoPago) · **3b.2** switch `compras.js` a proveedor UUID (pasada dedicada, riesgoso) · remito simple (a charlar — pisa con el remito de Transporte) · retiro legacy de `cargas` (destructivo, confirmar) · bloqueo de asientos retroactivos por período (opcional) · activación real 2027 (cargar saldos + RPC = tarea de Fede).
