...
- **🔔 Notificaciones por evento y rol (NUEVO — pedido de Fede 2026-07-04):** racionalizar la matriz de qué evento operativo notifica a qué rol y por qué mecanismo (notif push / alerta calculada / tarea derivada), y llenar los gaps. Hoy es inconsistente (algunos eventos notifican, otros alertan, otros nada; casi todo es client-side sin trigger SQL de respaldo). **Gaps clave:** OC recibida incompleta→admin · equipo fuera de servicio→admin+taller · flota vencimiento (falta la tarea)→admin/taller · stock cruza mínimo→admin (hoy solo polling) · pago proveedor vencido→admin · vencimientos admin→admin · proyecto trabado→pm. Preferir triggers SQL para lo crítico. **Matriz completa + inventario de lo que YA notifica + gaps + fixes → `docs/handoff-capa-operativa-pulido-notificaciones.md`.**
- **🐞 Fix Flota — gating read-only roto:** `pm`/`taller` pueden crear/editar/borrar vehículos y mantenimiento pese a estar marcados read-only (`flota.js` nunca llama `Data.isReadOnly`). Agujero de permisos → prioridad al retomar.
