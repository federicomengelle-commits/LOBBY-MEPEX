...
- ~~**🎨 CRM Bandeja — rediseño VISUAL**~~ ✅ **DISEÑADO + CONSTRUIDO 2026-07-19** (commit `5ef1f7a`, sesión en vivo con 6 mockups): Bandeja en **cards 2-col** (cliente protagonista, acciones inline, sin montos) + **Ficha de Caso v3** "sala de situación" (cabecera rica, Resumen IA estilo Gemini, cotización desplegable sin montos, Drive del caso, ruta `#crm/caso/<id>`, chip evento deep-link). Reviews de los 3 agentes nativos pasados (3 HIGH cazados y arreglados). SQL+pull+connector ✅ hechos por Fede; **resumen IA real andando en prod**. Ronda 2 de densidad/íconos pusheada (`b9f15e9`); header ✅ aprobado ("mejoró mucho"). **⏸ PAUSA DE CÓDIGO — próximo paso: sesión dedicada de DISEÑO DEL FLUJO del composer/mensajes** (Fede no entiende el paradigma registro-manual; quiere resumen SIEMPRE/auto + composer para MANDAR; dealbreakers en memoria `feedback_crm_flujo_dealbreakers`). **Handoff completo con prompt de arranque: `docs/handoff-crm-ficha-flujo.md`** — diagramas con flechitas/ifs, 2-3 versiones, mockups, recién después código. **Fase 2 (post-celu):** enviar WhatsApp real · countdown del evento · adjuntar COT/catálogo · "mandar al Centro de Tareas". **Pendiente corto conexo:** barrido global botón-volver (diseño listo, tarea autónoma).
