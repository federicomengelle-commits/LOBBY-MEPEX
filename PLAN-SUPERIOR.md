...
- **Triage de la ronda** → arreglar lo que el equipo cace → resto del catálogo `pulir-pantallas`: **Tareas ✅ · Inventario ✅ · Compras ✅** (2026-07-04). Quedan: Locaciones-operativa, CRM Bandeja visual, **Flota** (KPI/banner de vencimientos a nivel módulo).
- **🔑 Inventario v2 — SEGUIR (Fede: "no lo veo final, pensémoslo bien"):** hecho hoy = shell estándar + Dashboard launchpad + KPIs en 5 subtabs + **discriminación de stock** (Piezas 226→**217** sin servicios/subalq, Materiales 80→**42** sin subalquiler; regla `_soloPiezasFisicas`/`_soloMaterialesReales`) + Movimientos (saca filtro Proyecto + acento) + 🐞 fix stock materiales. **FALTA:** (a) **Físico rework COMPACTO** (misma tabla que le gusta, simplificar — NO cards grandes; chips para contar por partes + Esperado/Contado + saca Código roto; "por galpón" no mapea). (b) **Dashboard expandir** (definir qué señal va arriba). **Método: mockup→OK→aplicar, sin romper.** 👉 `docs/handoff-proxima-charla-plan-superior.md`.
