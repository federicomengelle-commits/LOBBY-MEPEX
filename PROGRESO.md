# PROGRESO — Rediseño LOBBY-MEPEX  ·  AVANCE ≈ 57%

> **Registro de lo YA HECHO.** Lo que FALTA vive en `PLAN-MAESTRO-rediseno-lobby.md` (≈43%).
...
- **🆕 Fase 7 CRM — E1 ARRANCADO (handover 2026-06-13):** ✅ `sql/crm_casos.sql` **CORRIDO** (3 tablas + `cotizaciones.caso_id` + migración `interacciones`→`crm_mensajes` + RLS comercial) · ✅ endpoint IA `tools/vps/crm-digest.js` escrito (driver gemini|claude, sin deployar). **PRÓXIMO (orden) → ver `docs/crm-casos-runbook.md` §ESTADO ACTUAL:** (1) Claude construye+pushea el **tab Casos** (front; SQL ya está) → (2) Fede pull+prueba → (3) key Gemini → (4) deploy endpoint en proxy → (5) WhatsApp pegado con IA = E1 cerrado → (6) E2 email.
- **⚠️ Deploy gap VPS:** venía atrasado en pulls (Tareas v2/v3/v4, lobby v7 iso 84px). **Pull general recomendado** para sincronizar.
- **Última actualización:** 2026-06-13h (cierre de sesión + handover — CRM E1 arrancado: SQL corrido + endpoint IA escrito; tab Casos = próximo. Antes en esta sesión: Fase 9.bis RLS cerrada · Fase 11 Tareas v4 · CRM runbook · iso lobby). Próximo macro a elegir por Fede: **Fase 7 CRM "Casos"** (WhatsApp/Gmail/IA) · **Fase 8 Finanzas** (auditoría + "Rendimiento por evento" que desbloquea RRHH.5) · **Fase 4 Logística-remito** (espera cotizador) · **Fase 5 enhancements** · **Fase 10 Remate UI**. Próximo macro a elegir por Fede: **Fase 7 CRM "Casos"** (WhatsApp/Gmail/IA) · **Fase 8 Finanzas** (auditoría + "Rendimiento por evento" que desbloquea RRHH.5) · **Fase 4 Logística-remito** (espera cotizador) · **Fase 5 enhancements** · **Fase 9.bis Roles & Permisos**. Pendiente de Fede: pull en VPS + verificación visual de las 4 tabs de RRHH + instalación guiada del loop de deploy + correr los DROP comentados de `sql/rrhh2_ausencias.sql` (con backup).
