...
- **🆕 Fase 7 CRM — E1 núcleo construido (2026-06-13) + REFACTOR v2 decidido (2026-06-14):** ✅ `sql/crm_casos.sql` CORRIDO · ✅ endpoint IA `tools/vps/crm-digest.js` escrito (sin deployar) · ✅ **núcleo CONSTRUIDO+pusheado** (`crm.js?v=14`/`api.js?v=38`): bandeja + ficha full-screen (timeline unificado + composer 4 canales) + WhatsApp pegado con `API.crmDigest`/**fallback parser local** + **pegar capturas al historial** + @menciones→notif. Revisado adversarialmente (workflow) + verificado en preview (carga sin errores, parser, XSS escapado).
  ...
  - **🔴 BLOCKER (único para cerrar E1):** el **browser NO llega al `:3000`** (`Failed to fetch`); **lapyme también falla** → es la red/proxy del :3000, no el código. Fix recomendado: rutear `/api/` por **nginx** (proxy_pass 127.0.0.1:3000) + `CRM_DIGEST_URL` relativo. Mientras tanto el WhatsApp pegado cae al **parser local (modo manual)**, anda igual. Diagnóstico + fix detallado en `docs/crm-casos-runbook.md` §ESTADO ACTUAL.
  - **PENDIENTE Fase 7:** resolver el blocker del :3000 → R4 (listas de difusión, E3; listmonk ya está en el VPS) · link reverso cotización→caso · plan de cobro auto al convertir · afinar prompt del digest (inventa fecha si el chat no la trae) · E2-E5 ingesta.
