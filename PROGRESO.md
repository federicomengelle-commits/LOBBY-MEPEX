...
- **🆕 Fase 7 CRM — E1 EN CURSO (tab Casos construido 2026-06-13):** ✅ `sql/crm_casos.sql` **CORRIDO** · ✅ endpoint IA `tools/vps/crm-digest.js` escrito (sin deployar) · ✅ **TAB "CASOS" CONSTRUIDO+pusheado** (`crm.js?v=14`/`api.js?v=38`): bandeja de hoy (KPIs+lista priorizada+sin-asignar) · pipeline kanban de casos · ficha (timeline unificado WhatsApp/email/nota/llamada/sistema + composer 4 canales + panel lateral) · WhatsApp pegado con `API.crmDigest` + **fallback parser local (modo manual)** · **pegar capturas/imágenes al historial** (paste/📎 → downscale → `crm_mensajes.adjuntos` data URL, sin bucket) · @menciones→notif. Tab = **"Casos"**; **Interacciones se dejó intacto** (read-only legacy — decisión de Fede). Revisado adversarialmente (workflow). **PRÓXIMO → `docs/crm-casos-runbook.md` §ESTADO ACTUAL:** (1) Fede pull general VPS + prueba → (2) key Gemini → (3) deploy endpoint en proxy → (4) WhatsApp con IA = E1 cerrado → (5) E2 email.
- **⚠️ Deploy gap VPS:** venía atrasado en pulls (Tareas v2/v3/v4, lobby v7 iso 84px). **Pull general recomendado** para sincronizar.
- **Última actualización:** 2026-06-13i (CRM Fase 7 E1 — **tab "Casos" construido**: bandeja+pipeline+ficha+timeline+composer 4 canales+WhatsApp pegado con fallback local+pegar imágenes+@menciones. `crm.js?v=14`/`api.js?v=38`. Próximo: pull VPS + probar → key Gemini → deploy endpoint). Próximo macro a elegir por Fede: **Fase 7 CRM "Casos"** (WhatsApp/Gmail/IA) · **Fase 8 Finanzas** (auditoría + "Rendimiento por evento" que desbloquea RRHH.5) · **Fase 4 Logística-remito** (espera cotizador) · **Fase 5 enhancements** · **Fase 10 Remate UI**. Próximo macro a elegir por Fede: **Fase 7 CRM "Casos"** (WhatsApp/Gmail/IA) · **Fase 8 Finanzas** (auditoría + "Rendimiento por evento" que desbloquea RRHH.5) · **Fase 4 Logística-remito** (espera cotizador) · **Fase 5 enhancements** · **Fase 9.bis Roles & Permisos**. Pendiente de Fede: pull en VPS + verificación visual de las 4 tabs de RRHH + instalación guiada del loop de deploy + correr los DROP comentados de `sql/rrhh2_ausencias.sql` (con backup).
