...
- **🆕 Fase 8 — "Rendimiento por evento" ✅ CONSTRUIDO (2026-06-18, SQL-first · pendiente que Fede corra el SQL + verificación end-to-end):** módulo propio `#rendimiento` (ADMIN & FINANZAS, gateado por permiso `rendimiento` + RLS `fn_role_can('finanzas')`, admin/superadmin). Reemplaza el Excel de pagos de Lelean. **Archivos:** `rendimiento.js?v=1` (nuevo) · `api.js?v=42` (bloque RENDIMIENTO: `createEgreso` genérico + `createComprobanteRecibido` + `pagarCostoEvento` orquestador + CRUD catálogo/costos/pagos/materiales + dashboard + comparativa + `getJornalesByPersona`) · `data.js?v=15` (módulo en categoría admin + rolePermissions) · `router.js?v=11` (ruta) · `sql/rendimiento_evento.sql` (DDL idempotente: `evento_costo_catalogo`+`evento_costos`+`evento_costo_pagos`+`evento_rendimiento` + trigger `fn_sync_costo_desde_pago` + RLS + grant rol + gates). **Funciona:** planilla 5 categorías colapsables (jornal/flete/proveedor/seguro/comida) con autocomplete del catálogo (tarifa default editable → badge "editado") + `monto_previsto` · pagos SIEMPRE discriminados (1 pago = 1 egreso → asiento auto; "Pagar seleccionados" = N egresos, NUNCA consolidado) con adelantos/tandas (pendiente→parcial→pagado) · proveedor con factura → comprobante recibido (IVA→Libro IVA) + link bidireccional · dashboard Cobrado + Facturado lado a lado − Costos − Materiales = Ganancia + margen + waterfall · materiales carga manual · duplicar planilla · comparar eventos (ranking, superadmin) · contrato RRHH.5 (`getJornalesByPersona`). **⛔ Pasos de Fede antes de usar:** (1) correr `sql/rendimiento_evento.sql` en Supabase (incluye grant del permiso `rendimiento` a superadmin/admin) · (2) verificar GATE A (5 categorías egreso en `mapeo_cuentas`) y GATE B (CHECK de `egresos.categoria` acepta rrhh/logistica/servicio/otro) · (3) `~/pull-lobby.sh` · (4) test end-to-end. **Desbloquea RRHH.5.** **Deudas declaradas (no del módulo, NO bloquean):** fix global IVA en `fn_asiento_auto_egreso` (3ª línea crédito fiscal) + anular egreso no revierte asiento. Ver §Fase 8 en PLAN-MAESTRO + `docs/modulo-rendimiento-evento-blueprint.md`.
