...
- **Próximo (Fase 4) — ⛔ los 2 ítems que siguen están GATEADOS POR DECISIONES DE FEDE** (verificado en prod 2026-06-08, detalle en `PLAN-MAESTRO` §Fase 4):
  - **(a) Subalquileres por proveedor** → bloqueado por DATA: el modelo existe (stand→cotización→`cotizacion_items`→`catalogo_items` subalq→proveedor) pero `cotizacion_items` está **vacía** porque el cotizador del VPS no escribe ahí. **Decidir:** ¿el cotizador escribirá `cotizacion_items`?
  - **(b) Logística / 3.1b** → no es repunte suelto, es el rediseño: consolidar en `cargas`+`vehiculos`, retirar `logistica_*` legacy, **badge de vehículos → Flota**, VTV/seguro ya viven en `produccion_mantenimiento`. **Decidir:** ¿confirmás la dirección propuesta?
  - **Desbloqueado para avanzar mientras tanto:** notificaciones unificadas (Fase 9), auditoría de cambios CRM (Fase 7), o el rediseño de Logística si confirmás la dirección. Después Fases 5–10.
