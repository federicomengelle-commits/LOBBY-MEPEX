# PROGRESO — Rediseño LOBBY-MEPEX  ·  AVANCE ≈ 60%

> **Registro de lo YA HECHO.** Lo que FALTA vive en `PLAN-MAESTRO-rediseno-lobby.md` (≈40%).
...
- **🆕 Fase 7 CRM — E1 núcleo construido (2026-06-13) + REFACTOR v2 decidido (2026-06-14):** ✅ `sql/crm_casos.sql` CORRIDO · ✅ endpoint IA `tools/vps/crm-digest.js` escrito (sin deployar) · ✅ **núcleo CONSTRUIDO+pusheado** (`crm.js?v=14`/`api.js?v=38`): bandeja + ficha full-screen (timeline unificado + composer 4 canales) + WhatsApp pegado con `API.crmDigest`/**fallback parser local** + **pegar capturas al historial** + @menciones→notif. Revisado adversarialmente (workflow) + verificado en preview (carga sin errores, parser, XSS escapado).
  - **🔥 REFACTOR v2 (en curso, plan en PLAN-MAESTRO §Fase 7 + `docs/crm-casos-blueprint.md` §14):** Fede detectó 2 pipelines redundantes (cotizaciones + casos) y el panel lateral angosto como patrón malo. Decisiones: **tabs→5 planas** (Bandeja·Pipeline·Clientes·Cotizaciones·Analítica; "Casos" se disuelve, **Interacciones se jubila**) · **Pipeline=kanban único de casos con DnD** (reemplaza el de cotizaciones) · **full-screen en todo** · backfill `sql/crm_casos_backfill.sql` (1 caso x cliente+evento). Fases R1 (unificación)→R2 (clientes full-screen)→R3 (conexión Finanzas)→R4 (listas de difusión). **PRÓXIMO INMEDIATO: 🧑‍💻 correr `sql/crm_casos_backfill.sql` → 🤖 R1.** IA real (key Gemini + deploy `/api/crm/digest`) sigue pendiente, se cierra después del refactor.
- **⚠️ Deploy gap VPS:** venía atrasado en pulls (Tareas v2/v3/v4, lobby v7 iso 84px). **Pull general recomendado** para sincronizar.
- **Última actualización:** 2026-06-14 (CRM Fase 7 — E1 núcleo construido + **REFACTOR v2 decidido** con Fede: un solo Pipeline de casos + full-screen + tabs 5 planas + Interacciones jubilada + conexión Finanzas + listas de difusión. Spec en blueprint §14. Backfill `sql/crm_casos_backfill.sql` escrito. **Próximo: Fede corre el backfill → R1.**). Próximo macro a elegir por Fede: **Fase 7 CRM "Casos"** (WhatsApp/Gmail/IA) · **Fase 8 Finanzas** (auditoría + "Rendimiento por evento" que desbloquea RRHH.5) · **Fase 4 Logística-remito** (espera cotizador) · **Fase 5 enhancements** · **Fase 10 Remate UI**. Próximo macro a elegir por Fede: **Fase 7 CRM "Casos"** (WhatsApp/Gmail/IA) · **Fase 8 Finanzas** (auditoría + "Rendimiento por evento" que desbloquea RRHH.5) · **Fase 4 Logística-remito** (espera cotizador) · **Fase 5 enhancements** · **Fase 9.bis Roles & Permisos**. Pendiente de Fede: pull en VPS + verificación visual de las 4 tabs de RRHH + instalación guiada del loop de deploy + correr los DROP comentados de `sql/rrhh2_ausencias.sql` (con backup).
