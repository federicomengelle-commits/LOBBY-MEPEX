...
const ProyectoDetalle = {
    ...
    async _toggleChecklistItem(itemId, willCheck) {
        ...
        // Va ANTES del return de abajo: con un checklist de un solo ítem, tildarlo
        // cambia el estado Y completa al 100% en el mismo click.
        ...
        if (p.estado_taller !== prevEstadoTaller) {
            if (!(await this._loadProject())) return;   // ídem: sin proyecto, no se arma el shell
    ...
    _attachShellEvents() {
        ...
        document.querySelectorAll('.pjd-cs-btn[data-ciclo]').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                // Guard: `_loadProject()` deja `_project` en null si falla, y
                // `_buildShell()` desreferencia `p.estado` en la primera línea. Un blip
                // de red justo después de un guardado exitoso tiraba un TypeError.
                if (!(await this._loadProject())) { Toast.warning('Se guardó, pero no se pudo refrescar la pantalla. Recargá.'); return; }
    ...
    async _changeStatus(newStatus) {
        try {
            ...
            // Mismo guard que arriba: éste ya existía sin él desde antes de la tanda 7.
            if (!(await this._loadProject())) { Toast.warning('Se guardó, pero no se pudo refrescar la pantalla. Recargá.'); return; }
