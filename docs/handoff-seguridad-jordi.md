...
3. **Claude para PII** — ✅ **RESUELTO 2026-07-11** (falta solo el deploy): Fede creó la `ANTHROPIC_API_KEY` (validada + driver probado end-to-end en local: digest parsea perfecto, OCR shape aceptado). Los 3 tools ya traían el driver `claude` (default `claude-haiku-4-5-20251001`, centavos). `server.js` ahora monta también `/api/crm/digest` (iaLimit + requireAuth; el front ya manda Bearer). **Deploy:** agregar `MODEL_PROVIDER=claude` + `ANTHROPIC_API_KEY=…` al `/home/mepex/api/.env` → pull → `cp server.js crm-digest.js ocr-comprobante.js` a `/home/mepex/api/` → `pm2 restart mepex-api`. La key vive SOLO en ese .env (jamás en el repo).
