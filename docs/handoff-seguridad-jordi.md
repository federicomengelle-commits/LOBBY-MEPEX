...
4. **2 policies anon opcionales** (PASO 2 de `sql/seguridad_rls_fixes.sql`): `clientes` legible por anon (¿el cotizador la usa con la anon key? si no, dropear) + `encuestas_evento` editable por anon (mover el submit a un RPC `SECURITY DEFINER` y quitar el UPDATE anon — toca `encuesta.html`).
