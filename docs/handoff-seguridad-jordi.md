...
3. **Claude para PII** (opcional, decisión de costo): hoy el CRM digest + OCR mandan PII de clientes a **Gemini free tier** (que entrena). No hay `ANTHROPIC_API_KEY` en el proxy. Para usar Claude (no entrena): crear key en console.anthropic.com (paga, centavos) → `MODEL_PROVIDER=claude` + `ANTHROPIC_API_KEY=…` en `/home/mepex/api/.env` → `pm2 restart mepex-api`. Alternativa código: minimizar la PII enviada. (Vertex/Gemini no-training está bloqueado por el mismo tema GCP del Gmail.)
4. **2 policies anon opcionales** — ✅ **RESUELTO 2026-07-11** (código+SQL listos, falta correr): el cotizador **NO** usa la anon key (todo por su server con service_role, verificado) y `encuesta.html` se migró a RPCs `SECURITY DEFINER` (`fn_encuesta_publica_get` / `fn_encuesta_publica_responder`: token validado server-side, un solo uso, respuestas sanitizadas). **Orden estricto:** correr `sql/seguridad_rls_paso2_encuesta_rpc.sql` → pull → smoke de un link de encuesta real → correr `sql/seguridad_rls_paso2b_drop_anon.sql` (dropea `clientes_rls_anon`, `encuestas_rls_anon_sel`/`_upd` y `eventos_anon_select` + verificación).
5. **Gemini key al header** en `crm-digest.js`/`octexa-ia.js`: editado en repo; `ocr-comprobante.js` ya está en el VPS. crm/octexa no están montados en el `server.js` actual (no urgente).
