...
- **El toggle "Vende" hoy sólo lo puede tocar Fede.** `#admin-panel` es `superadminOnly`, pero el trigger de la DB acepta admin **o** superadmin. La asimetría es segura (el cliente es más restrictivo que la base), pero significa que si mañana entra alguien al equipo comercial, tiene que marcarlo Fede — Lelean no puede aunque la DB se lo permitiría. Cambiarlo es un ajuste de la ruta, no del SQL.
