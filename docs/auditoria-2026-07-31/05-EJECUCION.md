...
| **T3.13** | `requireRole` sin el rol `finanzas` inexistente | VPS | 🟡 | editado en el repo · ⏳ llega con el deploy de **T2** |
...
| **T3.24** | Barrido de las ~41 fechas en UTC que quedan | JS | ⬜ | nuevo, salió de T3.20. ⚠️ **El peor: `finanzas.js:7940` manda esa fecha a ARCA como fecha del comprobante** → emitir después de las 21:00 fecha la factura **fiscal** un día adelantada |
...
| **T4.11** | `_safeUrl` global + los 10 `href` sin validar | JS | ⬜ | |
| **T4.12** | `crm.js._escHtml` inseguro en ~23 atributos | JS | ⬜ | XSS vivo |
