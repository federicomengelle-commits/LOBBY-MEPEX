...
| **T0.9** | FK de `plan_cobro.proyecto_id` + huérfanos de $10,7M | SQL | 🟡 | **los huérfanos se disolvieron con T5.2-bis**: eran los 5 cobros semilla del 06/04 + el egreso Coolskin, todos anulados el 5/8. Queda sólo la parte de schema: **agregar las 2 FKs** (`ingresos.proyecto_id`, `egresos.proyecto_id`) + la de `plan_cobro.proyecto_id`, que ahora se pueden crear porque no hay filas que las violen. Es DDL |
