...
| **T0.9** | FK de `plan_cobro.proyecto_id` + huérfanos de $10,7M | SQL | 🟡 | **ingresos y egresos ✅** (2 FKs `ON DELETE SET NULL`, aplicadas por MCP el 5/8; 0 huérfanos, los 9 movimientos con proyecto vivo intactos). ⛔ **`plan_cobro` afuera**: el `CHECK chk_plan_cobro_tiene_dueno` rechaza dejar `proyecto_id` y `venta_id` los dos en NULL, y la fila huérfana tiene `venta_id` NULL → hace falta **borrarla físicamente**, que es destructivo y decisión de Fede. Ver **T0.9b** |
| **T0.9b** | Lo que quedó abierto del frente de FKs | SQL | ⬜ | (1) la FK de `plan_cobro`, tras resolver la fila `8c2b86c9`. (2) **5 tablas más con `proyecto_id` y ninguna FK**, verificado en prod: `comprobantes`, `comprobantes_recibidos`, `cartera_valores`, `crm_casos`, `compras_ordenes`. Hoy sin huérfanos —por eso no salieron en el tablero del 31/07, que contó relaciones CON filas rotas— pero es el mismo hueco que causó T0.9 |
