...
| **T0.9b** | 5 tablas más con `proyecto_id` y ninguna FK | SQL | ⬜ | `comprobantes`, `comprobantes_recibidos`, `cartera_valores`, `crm_casos`, `compras_ordenes`. Hoy sin huérfanos —por eso no salieron en el tablero del 31/07, que contó relaciones CON filas rotas, no relaciones sin constraint— pero es el mismo hueco que causó T0.9. Las dos primeras son las calientes: Facturación escribe ahí todos los días |
