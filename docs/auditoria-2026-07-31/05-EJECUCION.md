...
| **T4.19** | **El motor de costos viejo, vivo por duplicado** | JS | ⬜ | **nuevo, 2026-08-02.** Sale del cruce T3.2 × T3.23 × la línea 252 del `04e`. Ver la nota abajo — **es una mina para el gate de costos de PUESTA-A-PUNTO** |
...
## ⚠️ T4.19 — el motor de costos viejo sigue vivo, por duplicado (nuevo 2026-08-02)
...
**El fix es borrar las tres, no arreglar el typo.** Después de sacar el bloque de `costos.js` y la definición duplicada, `recalcularCostoItem` queda con **cero llamadores** (mismo criterio con el que T3.23 sacó `recalcularPrecioAlquiler`). Es borrado de código muerto y probadamente inalcanzable, pero toca el archivo del costeo → **va con su propia revisión y con OK de Fede**, no colgado de otro ítem.
