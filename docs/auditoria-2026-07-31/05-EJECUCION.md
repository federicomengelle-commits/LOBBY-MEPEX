...
| **T0.8** | Índices únicos en comprobantes (emitidos y recibidos) | SQL | ✅ | **los 3 aplicados.** El de recibidos se aplicó el 2026-08-05 por MCP tras la limpieza de T5.2, y se probó con rollback: dos INSERT idénticos → el 2º rebota con 23505 |
| **T0.9** | FK de `plan_cobro.proyecto_id` + huérfanos de $10,7M | SQL | ✅ | **las 3 FKs aplicadas** (`ON DELETE SET NULL`, por MCP el 5/8). El tramo de `plan_cobro` se destrabó al borrar la fila dummy `8c2b86c9` con OK de Fede (*"es todo mentira"*) — sin eso, el `CHECK chk_plan_cobro_tiene_dueno` la dejaba impasable. Verificado: 0 huérfanos, 0 cuotas sueltas, los 9 movimientos con proyecto vivo intactos |
| **T0.9b** | Tablas con `proyecto_id` y ninguna FK | SQL | ✅ | **eran 8, no 5.** La lista escrita a mano tenía `comprobantes`, `comprobantes_recibidos`, `cartera_valores`, `crm_casos` y `compras_ordenes`; un barrido sobre `pg_attribute` sumó **`compras_pedidos`, `inventario_movimientos` y `tareas`**, las tres con datos. Las 8 aplicadas el 5/8 (0 huérfanos, columnas UUID, sin CHECK que muerda). **Estado final: 24 tablas con FK a `proyectos` y 3 sin ella** — las 3 legacy vacías del `DROP_CHECKLIST`, a propósito |
...
| **T5.1** | Recalcular el ítem 89 | dato | 🔶 | **no es 1 clic**: el clic aplica una vida útil de 5 usos que nadie validó. **El rediseño de criterios YA SE HIZO** (2026-08-06, `docs/costos-modelo-decidido.md`) y el ítem quedó **excluido a propósito** de la política de márgenes. Ahora se sabe *por qué* con precisión: su costo por uso guardado ($10.197) es **menor** que el del panel blanco ($14.041), lo cual es imposible — misma receta exacta, mitad de vida útil, así que su costo tiene que ser el **doble** ($28.082). Sigue congelado hasta que Diego conteste si el panel negro dura de verdad la mitad (`docs/costos-preguntas-taller.md`). **Dato que reordena la urgencia: el panel negro se cotizó 1 vez, 1 unidad; el blanco 2.247** |
...
| **T5.3** | Cargar `costo_dia_referencial` (0 de 24) | dato | 🟡 | ⚠️ **después de T3.3**. **En curso desde el 2026-08-06:** planilla `MEPEX_personal_jornales.xlsx` enviada a Lelean, pre-llenada desde prod; pide **jornal + rol + teléfono + fecha de ingreso de un solo viaje**, con hoja `_sistema` que mapea fila→UUID (hay dos Diego y dos Hernández). Vuelve completada y la carga la hace Claude por MCP. **El DNI no se pidió: 22 de 24 tienen CUIL y el documento son los 8 dígitos del medio.** ⚠️ **La columna es `personas.costo_dia_referencial`** — existe una segunda, `personas.jornal_diario`, **muerta**: 0 filas y ningún lector, anotada para DROP |
| **T5.4** | ~~Cargar `stock_minimo` (0 de 80)~~ → **relevar el stock físico** | dato | ⬜ | **Reformulado el 2026-08-06 al medir el otro lado del cálculo.** La alerta dispara con `stock < stock_minimo`, y **79 de 80 insumos tienen `stock = 0`** (cero, no nulo). Cargar los 80 mínimos no enciende una alerta apagada: **enciende 79 alertas rojas a la vez**, todas ciertas e inútiles, y la alarma nace desacreditada. Lo que falta de verdad es el **stock**, y eso es relevamiento físico del galpón — ya planificado como actividad de estreno de las tablets (etapa 5 de `PUESTA-A-PUNTO-2027`). El mínimo va **después** |
...
| **T5.8** | Depurar los 7 superadmins | dato | ✅ | **2026-08-06: 7 → 4.** Bajados Mariano Arga, Bruno Caruso y Luqui — los tres habían entrado **una sola vez, el día que se los creó** — + revocada la sesión viva de Luqui. Cada aviso "a los superadmin" pasa de 7 filas a 4. **Fede decidió que Jordi, Ana/`test@`, Lex@ y Budie/`test3@` quedan**; los cuatro necesitan MFA (ver T5.14). ⚠️ **`test@mepex.local` es hoy la cuenta más expuesta**: superadmin, activa, sin MFA, con login del 11/07 sin identificar |
| **T5.9** | ~~Instalar la PWA en celulares de taller y pm~~ → **armar el parque de dispositivos** | operativo | ⬜ | **Redefinido por Fede el 2026-08-06:** *"en los celulares de la gente no, son todos usuarios de PC"*. No son instalaciones individuales: son **2 celulares de la empresa + 3 tablets Android**, armados **desde la oficina** (~2 h). La guía vieja (`docs/guia-instalar-app-celular.md`) era de auto-instalación y no servía → procedimiento nuevo en **`docs/puesta-en-marcha-dispositivos.md`**. **Sigue en cero: las 4 suscripciones de push son 2 PC + 1 Mac + 1 iPhone, todas de superadmins — ningún Android.** El tótem TV va aparte y está **bloqueado por software**: la pantalla `#tablero` no existe (sin ruta, sin archivo, sin menú) |
| **T5.12** | Revocar las sesiones vivas de las 4 cuentas de baja | operativo | ✅ | Hecho por MCP el 5/8 con OK de Fede. **Era 1 sesión, no 4**: las cuentas de baja son `test2`/`prueba`/`holis`/`test4` y sólo `test2` tenía sesión viva. Verificado: 0 sesiones y 0 refresh tokens de cuentas de baja · las 18 sesiones de cuentas activas intactas |
| **T5.13** | Activar "Leaked password protection" | operativo | ✅ | **2026-08-06.** Activado por Fede y **verificado por advisor**: el aviso `auth_leaked_password_protection` desapareció (93 → 92 lints). ⚠️ **La ruta es `Authentication → Attack Protection`**, no "Sign In / Providers" — y el switch exige plan Pro, que la org tiene |
| **T5.14** | **MFA en las cuentas admin/superadmin** *(nace del T5.8)* | operativo | ⬜ | **Medido el 2026-08-06: de 8 cuentas activas, sólo Fede tiene MFA.** Faltan 6: Lelean, Sofi y Noe (admin) y **Jordi, Ana y Lex@ (superadmin)**. Como el T5.8 se resolvió dejando a los cuatro en duda, ya no hay nada que esperar. Orden: los 3 superadmin primero. Pre-requisito: Google Authenticator o Authy en el celular de cada uno |
| **T5.10** | Datos de prueba vivos (AAAAC, 6 mensajes, 4 tareas, 3 jornales) | dato | ✅ | 2026-08-05: 6 mensajes + 1 caso de "PRUEBA CLAUDE v4", 8 tareas manuales, la tarifa inventada de $200 → 0, y **AAAAC** dado de baja al cerrar T5.2-bis (su ingreso de $1,5M ya estaba anulado) |
...
## ✅ T5.2-bis — **el libro contable era data de prueba** (cerrado el 2026-08-05)
...
### Lo que se hizo — `sql/auditoria_t5_2bis_blanqueo_libro.sql`

Fede eligió blanquear, y sobre si borrar también los dos comprobantes de Olavarría dijo *"decime vos"*. **Se quedan**: son lo único con CAE real de AFIP y `comprobantes` es el espejo local de lo emitido bajo el CUIT de MEPEX — una invariante que se rompe una vez por comodidad ya no es invariante. Además no ensucian ningún número (la NC neutraliza la factura; el IVA débito de junio ya daba $0,00 desde T4.5) y sirven como emisión buena de referencia cuando se vuelva a probar el circuito ARCA.

Se replicaron `API.anularCobro`/`anularPago` **al pie de la letra** (api.js:7205-7409), incluido el `_limpiarSatelite` que cuenta filas antes y después. **No se pudo usar el camino más directo**: el `javascript_tool` del Chrome corre en un **mundo aislado** — ve el DOM pero no los globals de la página, así que llamar `API.anularCobro` desde la consola no era una opción. Precondición verificada antes de arrancar: `cartera_valores` en 0 filas vivas → las ramas de cheque y endoso no aplicaban, y el script aborta si alguna vez dejan de estar en cero.

**14 movimientos anulados** (7 ingresos $15.201.000 + 7 egresos $2.784.007) · **13 contra-asientos** por trigger · **2 asientos manuales** de prueba dados de baja (#6 "Ajuste" y #9 "Pago por equipamiento", $500.000 c/u — no tienen movimiento detrás, ningún botón de Finanzas los alcanza) · **3 comprobantes recibidos** · **AAAAC** (cierra T5.10) · y **4 planes de cobro + 9 cuotas + 2 ventas**, que eran el último número vivo del tablero: **$39.000.000 de "Por cobrar"**.

**Verificado**: partida doble **$0,00** · 0 desbalanceados · 0 drift · **saldo final de cada cuenta $0,00** · las 9 cuotas volvieron a `pendiente` con $0 cobrado **recalculadas por el trigger, no por la mano** · y en pantalla, con la sesión real en Chrome: los 8 KPIs del Dashboard en $0,00, el pie de Ingresos en "Total: $0,00" con las filas en Anulado, y el Lobby sin un solo importe.

**No se tocó la capa operativa**: 11 proyectos, 7 eventos, 18 cotizaciones, personas, asignaciones y jornadas siguen enteras. Es data real de trabajo y es la base sobre la que se va a testear.

**Detalle a tener presente:** los contra-asientos van fechados **2026-08-05**, así que marzo-julio conservan el movimiento *y* su reversión. Es lo que hace el botón Anular: una reversión es un hecho con su propia fecha. El saldo acumulado de hoy es cero en todas las cuentas.
...
## TANDA 7 · Testeo integral end-to-end — **el último punto**

> Pedido de Fede el **2026-08-05**: *"hay que diseñar un plan para probar todas las variables… vos vas a hacer todo el testeo, vas a ser el tester oficial… y ver lo que no funciona y no impacta donde tenga que impactar, en todos los endpoints correspondientes. Vas a ser el auditor y vas a ir corrigiendo las cosas que encontrás."*
> **Va al final**: cuando esté cerrado lo que queda de `PLAN-SUPERIOR.md` y de este tracker.

**La precondición ya está cumplida** — es lo que habilitó T5.2-bis: la base quedó en cero y se pueden **crear y borrar cosas sin ensuciar nada**, que es justo lo que un testeo así necesita. Antes del 5/8 cada prueba dejaba residuo indistinguible del dato real.

### El corte: por CIRCUITO, no por pantalla

Recorrer pantallas encuentra lo que está roto a la vista. Lo que Fede pidió es lo otro: **que lo que se carga en un lado aparezca en todos los que corresponde**. Cada caso de prueba se escribe como *"creo X → tiene que impactar en A, B, C y D"* y falla tanto si rompe como si **no llega a alguno de los cuatro**. Ese es el modo de falla que más apareció en esta auditoría (T4.5: los KPIs de Recibidos sin firmar mientras los de Emitidos sí; T4.4: el EERR con los totales convertidos y los renglones no).

Cada caso cierra con **deshacer y verificar que volvió a cero** — que es, además, la prueba del camino de anulación, el menos transitado de todos.

| # | Circuito | Tiene que impactar en |
|---|---|---|
| 1 | **Venta**: caso CRM → cotización → venta → plan de pagos → factura ARCA → cobranza con retención | pipeline · ficha de venta · plan de cuotas · `comprobantes` con CAE · ingreso · asiento · Libro IVA Ventas · KPIs de Finanzas y del Lobby · crédito fiscal |
| 2 | **Compra**: pedido → OC → presupuestos → recepción → comprobante recibido → egreso | stock · `compras_ordenes` · asiento con IVA crédito · Libro IVA Compras · notificación al aprobador |
| 3 | **Evento**: crear → jornadas → asignar gente → transporte y remito → jornales → cierre y encuesta | calendario operativo · tablero del taller · `evento_costos` · rendimiento · NPS · reseña de Google |
| 4 | **Proyecto/taller**: BOM → `estado_taller` → novedades → fotos → entrega firmada | ficha · chip "listo para salir" · completitud · acta PDF · avisos al PM |
| 5 | **Costos**: insumo → receta → recálculo (RPC) → lista de precios → PDF | `calcular_receta` · snapshots · BOM jerárquico · el precio que lee el Cotizador |
| 6 | **Inventario**: movimientos · conteo físico · umbral de stock mínimo | stock · alerta y aviso al taller (T3.9, hoy sin dato cargado) |
| 7 | **Tesorería**: transferencia · cheque/e-cheq · endoso · clearing | cartera de valores · saldos · asientos de cada tramo del ciclo |
| 8 | **Contabilidad**: asiento manual · mapeos · libros IVA · reportes | partida doble · `saldos_mensuales` y su arrastre · EERR · balance |
| 9 | **Transversal**: los 28 tipos de notificación × la matriz de canales · tareas y rutinas · undo/redo · **los 5 roles** · PWA y push · RLS | la campanita, el celular, y sobre todo **qué ve y qué NO ve cada rol** — se prueba con un usuario real de cada uno |

### Reglas del testeo

1. **En prod, con datos propios y cleanup exacto** — el método que viene funcionando toda la auditoría.
2. **Se arregla lo que aparece**, en el momento, con reviewers antes de commitear (regla 19). Si el arreglo es grande, se anota y se sigue: el testeo no se detiene.
3. **Un hallazgo sin número no es un hallazgo.** Cuánto, cuántas filas, qué pantalla.
4. **Lo que no se puede probar se dice.** Un circuito que necesita un usuario `taller` real o un celular no se marca en verde por analogía.

### El entregable

Un informe **sintético**, en el formato que pidió Fede: una línea por hallazgo.

> **Encontré** — qué se ve · **Era** — la causa, con el número · **Lo corregí** — qué se tocó, o por qué queda abierto.
...
|---|---|---|
| 2026-08-05f | **Barrido de escape + barrido de schema código↔prod** | Tanda hecha con Fede afuera, eligiendo sólo cosas **sin decisión de diseño adentro** (su regla: los flujos de UX se diagraman antes de codear). **(1) Los 3 `_esc` caseros que no escapaban comillas** —`compras.js`, `notifications.js`, `proyecto-detalle.js`— ahora delegan en el `escHtml` global, con fallback idéntico. **El barrido primero, el fix después:** 82 llamadas a esos helpers y **UNA sola dentro de un atributo** (`proyecto-detalle.js:1111`, el `title` de la fecha de una novedad, que es un timestamp de la base → no explotable). O sea que el agujero **no estaba abierto**; lo que se cerró es la trampa cargada para el próximo `title="${this._esc(nombre)}"`. Verificado extrayendo los 3 helpers reales del archivo y corriéndolos con y sin el global: misma salida, los 5 caracteres. **(2) Barrido de schema:** se extrajo cada `.select()` del JS (75 tablas · 410 referencias a columnas · 24 embeds) y se probó **contra prod por el mismo camino que usa el cliente** (PostgREST con `limit=0`, que devuelve 400 nombrando la columna que falta). **Resultado: 0 mismatches y 0 embeds rotos.** Es la clase de bug que este repo marcó como la más valiosa —JS pidiendo una columna que una migración renombró— y el último barrido era de junio, con muchas migraciones en el medio. **Las 5 marcas iniciales eran ruido de mi propio extractor** (agarra el `.select()` siguiente dentro de una ventana de caracteres, y se cruzaba con otra query: un `.insert()`, una variable `sel`, y tres `select('*')`); verificadas una por una contra el código antes de reportar nada. Lo mismo el único embed que dio PGRST201: `profile:profiles` está **anidado** dentro de `responsables:proyecto_responsables(...)`, no colgando de `proyectos` — la query real corre limpia. **(3) Tres pendientes más resultaron fantasmas**, verificados: el bug del *"armado de 3 días que se vuelve de 1"* ya lo había cerrado la sesión del 30/07 con el guard `armadoEsMultiDia` (y los armados multi-día están intactos en prod); `cotizacion_propuestas` **ya tiene sus 4 policies** (la nota de T0.2 quedó vieja); y ninguna tabla quedó con dos FKs a `proyectos`, así que las 8 FKs de hoy no introdujeron ambigüedad en ningún embed. **⚠️ Lo que NO se construyó, a propósito:** la integración de las propuestas del cotizador. Al medirla apareció que **`cotizacion_id` está NULL en las 5 filas** y `ref` en 4 de 5 → no hay join confiable, así que "leer la tabla y colgar el PDF" era optimista: hay una decisión de diseño (matchear por ref / por nombre / pedirle al cotizador que escriba el id) y esas van con Fede. Bumps: compras 25 · notifications 15 · proyecto-detalle 23 · app 40. |
| 2026-08-05e | **T0.9 ✅ COMPLETO · T5.12 ✅ · las 15 cotizaciones dummy · ★ hallazgo de las PROPUESTAS** | Tanda de decisiones de Fede, todas ejecutadas y verificadas. **Cotizaciones**: de las 16 en borrador identificó **COT-2026-0003 (Riderchail)** como la única que se mandó a un cliente — quedó en `enviada`, las otras 15 de baja. Vivas: 3. El resto eran huellas de probar el cotizador, y se notaba en los datos: montos idénticos entre clientes distintos ($3.338.627 en Fenix y en Gambassi; $23.974.904 tres veces entre Hilados y Barcos) y dos EGEO 2 separadas por **$4**. **T0.9 cerrado**: Fede autorizó borrar la fila `8c2b86c9` (*"es dummy también, es todo mentira"*) — se verificó antes que no tuviera nada colgando (0 cobros aplicados, 0 ingresos atados, su única cuota ya soft-deleteada, y el plan declarando $8.000.000 contra una sola cuota de $4.000.000) → DELETE + la 3ra FK. **T5.12 cerrado, y el ítem estaba mal contado**: las cuentas de baja son `test2`/`prueba`/`holis`/`test4` pero **sólo `test2` tenía sesión viva**, no las 4. Cerrada por MCP; verificado 0 sesiones y 0 refresh tokens de cuentas de baja, con las **18 sesiones de cuentas activas intactas**. **★ Y un pedido nuevo que resultó barato:** Fede contó que el Cotizador arma **propuestas comerciales brandeadas con renders de stands** y que *"no se ven desde acá"* — se ven los presupuestos, no las propuestas, que es lo que el cliente efectivamente mira. Medido en el momento: **`cotizacion_propuestas` ya vive en esta misma base, con 5 filas**, y trae `cliente`, `evento`, `total`, `ref`, `cotizacion_id` y **`pdf_url` a Storage**. O sea que no hay que construir nada del lado del Cotizador ni tocar su contrato: es leer esa tabla. Único cuidado, ya documentado en T0.2: la tabla tiene **RLS y cero policies**, así que hoy el browser no la puede leer. Anotado como punto 0 de "después de la auditoría". |
| 2026-08-05d | **T0.9 parcial ✅ · deuda de T4.19 ✅ · CSV del Libro IVA ✅** | Tanda hecha con Fede afuera, sobre deudas ya anotadas. **T0.9** — el **sql-reviewer volvió BLOCK con un CRITICAL que yo no había visto y que verifiqué contra prod antes de aceptarlo**: `plan_cobro` tiene `CHECK chk_plan_cobro_tiene_dueno (proyecto_id IS NOT NULL OR venta_id IS NOT NULL)`, y el plan huérfano tiene **`venta_id` NULL** — o sea que hoy cumple el CHECK sólo porque `proyecto_id` no es NULL, aunque apunte a la nada (el CHECK valida NULL-idad, no existencia). Mi `UPDATE ... SET proyecto_id = NULL` lo dejaba con las dos en NULL → **23514** → abortaba la transacción entera y **ni siquiera se aplicaban las FKs de ingresos y egresos, que estaban sanas**. Y no alcanza con filtrar `_deleted`: un CHECK se evalúa igual sobre una fila soft-deleteada. Se aplicaron las 2 FKs limpias (`ON DELETE SET NULL`, con `DROP CONSTRAINT IF EXISTS` delante — el otro hallazgo, HIGH: Postgres no tiene `ADD CONSTRAINT IF NOT EXISTS` y el repo ya resolvió ese patrón en `cotizaciones_fks.sql`) y **`plan_cobro` quedó afuera**, porque salir de ahí pide un DELETE físico y eso se consulta. Verificado: 0 huérfanos, los 9 movimientos con proyecto vivo intactos, partida doble $0,00. El reviewer aportó además un MEDIUM que abre **T0.9b**: hay **5 tablas más con `proyecto_id` y ninguna FK** (`comprobantes`, `comprobantes_recibidos`, `cartera_valores`, `crm_casos`, `compras_ordenes`) — no salieron en el tablero del 31/07 porque ese contó relaciones *con filas rotas hoy*, no relaciones sin constraint. **Deuda de T4.19** — de los dos sitios anotados, **uno resultó código muerto**: no existe ningún header con `data-sort-col="costoProduccion"` y `_sortCol` sólo se setea desde un header clickeado, así que ese `case` es inalcanzable (queda repuntado a la cadena de fallback, no borrado, para que no sea una trampa si alguien engancha el header). El otro, `costos.js:3757`, **sí era real y con número**: el modal "Cargar receta base" mostraba `costoProduccion` crudo —la columna del motor viejo, que el motor vigente no escribe— y **10 ítems aparecían en $0 teniendo costo**, $336.393 en total; el peor, *Vitrina mostrador 1,00m*, con **$234.700 mostrándose como cero** justo en la pantalla donde se elige de qué ítem copiar la receta. **CSV del Libro IVA Compras** — la tabla y el CSV calculaban los importes por separado y ya habían divergido: el CSV no miraba `iva_total`, que es donde los registros **auxiliares** guardan su IVA. Medido: con IVA mixto exportaba **$16.800 en vez de $18.900** (se comía el tramo del 10,5%) y un auxiliar cargado por total exportaba **cero** — al contador le llegaba menos crédito fiscal del que muestra la pantalla de al lado. Cerrado extrayendo `_ivaComprasImportes`, que usan los dos. **Y de paso una columna "Origen"**: las filas auxiliares son extracontables y el CSV las mezclaba con las oficiales sin marcarlas, cuando en pantalla sí se distinguen con el badge AUXILIAR. **No se tocó el lado de Ventas**: su `discriminaIVA` parece roto pero la auditoría ya documentó que el resultado es el correcto para un Libro IVA y que "arreglarlo" lo volvería un bug real. Bumps: costos 39 · contabilidad 22 · app 39. |
| 2026-08-05c | **T0.8 ✅ COMPLETO · KPI de presupuestos ✅ · hallazgo del embudo comercial** | **El MCP de Supabase apareció a mitad de sesión** → se aplicó el DDL que había quedado para Fede: `ux_comprobantes_recibidos_cuit_tipo_numero`, con prueba funcional (dos INSERT idénticos, el 2º rebota con **23505**, rollback sin residuo — ojo que `concepto` es NOT NULL y no estaba en el INSERT de ejemplo del plan). **T0.8 cerrado.** Y escrito `sql/auditoria_t0_9_fks_proyecto.sql`, que ya no está bloqueado. **★ Lo que encontró Fede mirando el tablero, y es lo que más vale de la tanda:** el KPI "Presupuestos" del lobby marcaba **0** con 18 cotizaciones vivas. El widget contaba bien —*emitidas este mes que ya salieron de borrador*, y en agosto no hay ninguna: la última emisión es del 6/7— pero abajo había algo peor: **`cotizaciones.estado` tiene DEFAULT `'borrador'` NOT NULL y el Cotizador no escribe la columna**, así que toda cotización nace ahí y nada la promueve — **16 de 18 llevan en borrador desde mayo**, y las 2 que no lo están son las más viejas, movidas a mano antes de que el Cotizador estuviera en uso. No es un KPI roto: es el **embudo comercial entero que no puede encenderse** (el kanban del CRM con 16 apiladas en una columna, la conversión del rol venta, y el aging *"enviada hace >3 días"* de `crm.js:2691`). Misma familia que A29. El KPI pasa a "Presupuestos abiertos" con el número que se cuenta con la mano y el subtítulo diciendo dónde están parados (`16 · 16 en borrador · 1 ganada · 50% conv`): el embudo trabado deja de estar escondido detrás de un 0. **Reviewer: APPROVE con 1 hallazgo real, y es el mismo bug que el diff decía arreglar, reintroducido por la puerta de al lado** — `kpi-conversion` gateaba con `env`, que **incluye `enviada` y `en_negociacion`**, y el subtítulo las llamaba "cerradas" a todas; con los datos de hoy no se nota (no hay ninguna en enviada), pero el primer presupuesto mandado y sin respuesta reponía el `0%` falso **contando como derrota algo que sigue abierto**. Cerrado con `_COT_CERRADAS` como propiedad del módulo, compartida por los dos widgets: si cada uno define lo suyo, el tablero termina diciendo dos cosas del mismo dato. Verificado corriendo **las funciones reales extraídas del `lobby.js`** (y la constante leída del archivo, no copiada) contra la data de prod + 11 casos borde. **Lo que NO se tocó a propósito:** mover esas 16 de estado — si se enviaron al cliente o quedaron a medias es un hecho del negocio. Anotado como hallazgo abierto en el handoff, junto con `vendedor_id` NULL en las 18. Bumps: lobby 23 · app 37. |
| 2026-08-05b | **T5.2-bis ✅ · T5.10 ✅ · T0.9 🟡** | El blanqueo del libro, decidido por Fede en la misma sesión. **Lo primero fue chocar contra el candado que pusimos nosotros**: `fn_candado_mov_contabilizado` rechaza el soft-delete de un movimiento contabilizado y manda a Anular — probado contra prod antes de tocar nada, y es la razón de que esta tanda saliera bien en vez de dejar 13 asientos huérfanos descontando saldo. **Segundo bloqueo, medido y esquivado:** el `javascript_tool` de Chrome corre en un **mundo aislado** (ve el DOM, no los globals de la página), así que llamar `API.anularCobro` desde la consola de su sesión —el plan A, el que usa el código real— no era posible. Plan B: replicar `anularCobro`/`anularPago` (api.js:7205-7409) **al pie de la letra**, con el `_limpiarSatelite` que cuenta filas antes y después, y con un guard que aborta si `cartera_valores` deja de estar en cero (las ramas de cheque y endoso no se replicaron **a propósito**: no había con qué probarlas). **Alcance:** 14 movimientos anulados ($15.201.000 + $2.784.007) → 13 contra-asientos por trigger · 2 asientos manuales de prueba (#6 y #9, $500.000 c/u) que **ningún botón de Finanzas alcanza** porque no tienen movimiento detrás · 3 comprobantes recibidos · AAAAC · y **4 planes de cobro + 9 cuotas + 2 ventas**, que eran el último número vivo del tablero: **$39.000.000 de "Por cobrar"**, sin un solo asiento detrás. **La decisión que Fede me delegó** (*"decime vos"*): los dos comprobantes de Olavarría **se quedan** — son lo único con CAE real, `comprobantes` es el espejo local de lo emitido bajo el CUIT de MEPEX, y no ensucian ningún número porque la NC ya neutralizaba la factura. **Verificación**: partida doble $0,00 · 0 desbalanceados · 0 drift · **saldo final de cada cuenta $0,00** · las 9 cuotas de vuelta a `pendiente` **recalculadas por `fn_sync_cuota_desde_aplicacion`, no por la mano** · y en pantalla, con la sesión real: los 8 KPIs del Dashboard en $0,00, el pie de Ingresos en "Total: $0,00" y el Lobby sin un importe, con agenda, montajes y proyectos intactos. **Lo que NO se tocó a propósito:** la capa operativa (11 proyectos, 7 eventos, 18 cotizaciones, personas, asignaciones) — es data real y es la base del testeo. **Y quedó anotada la TANDA 7**, el plan de testeo integral que pidió Fede en esta misma sesión. |
