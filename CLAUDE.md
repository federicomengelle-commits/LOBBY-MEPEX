...
> **Reorg capa operativa física = 100% CONSTRUIDA Y PUSHEADA (A→F + corte destructivo).** Módulos **Taller y Logística DISUELTOS** (el **rol** taller se conserva): Taller→Proyectos(galpón read-only)+Tareas, Logística→Transporte en la ficha del Evento; redirects `#taller→#tareas`/`#logistica→#eventos`. `taller.js` borrado; `logistica.js` inerte; tablas legacy `cargas`/`logistica_*` NO dropeadas. Tracking canónico en `PROGRESO.md` §sesiones 2026-06-24/25/25b + `PLAN-MAESTRO §REORGANIZACIÓN DE LA CAPA OPERATIVA FÍSICA` + handoff `docs/reorg-capa-operativa-handoff.md`. Spec: `docs/capa-operativa-blueprint.md`. **⏳ Fede: pull** (+ correr `sql/reorg_f_rutinas.sql` si no lo hizo) + verificar en prod como taller. Pulidos opcionales (no funcionales): borrar `logistica.js` del disco; limpiar `roles.permissions` del DB de los módulos inertes.
