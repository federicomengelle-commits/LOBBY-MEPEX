...
- **Sesión 2026-07-31b (adjunto del certificado de retención — hueco 4.1 del handoff) — SIN SQL QUE CORRER · ⏳ Fede: `~/pull-lobby.sh` + el `cp push.js` que sigue pendiente:** verificado al arrancar que **el pull ya estaba hecho** (prod sirve las mismas versiones que `App._APP_SCRIPTS`) pero **el `cp push.js` no** (`/api/push/aviso` → 404 vs `/api/push/tarea` → 401): los avisos de la matriz llegan a la campanita y **no al celular**. **Construido:** input de archivo en la grilla de retenciones de `cobranza.js` + subida al bucket **`comprobantes` ya existente** bajo prefijo `retenciones/` (verificado contra prod: existe, policies aplicadas, el INSERT no restringe path, y el límite de 15 MB + allowlist de mime del cliente son **exactos** a los del bucket) → **cero SQL**. **El libro nunca habría mostrado el clip:** `_safeUrl` exige `^https?://` y Storage guarda un **path** → `creditos-fiscales.js` resuelve las signed URLs **en batch antes de pintar** (resolverlas al click las mata el bloqueador de popups: post-`await` el `window.open` deja de ser gesto del usuario). Mismo hueco del otro lado: `_syncPercepciones` ahora **hereda** el `archivo_url` de la factura de compra, que es el respaldo de la percepción. **🔴 XSS vivo preexistente cerrado de paso:** `comprobantes_recibidos.archivo_url` (input de texto libre) iba **crudo** al `href` en `finanzas.js:9359` y sin escapar al `value` del modal; además el clip apuntaba a un path relativo → **404 en todo comprobante cargado por foto**. **Verificado: 2 harnesses, 72 checks, 0 errores.** Lo cazado por la verificación: un archivo **sin extensión** se guardaba como `certificado.sinex` (`'sin-extension'.split('.').pop()` devuelve el **nombre entero**) → la extensión ahora sale del **mime resuelto**, no del nombre. **Reviewers: security APPROVE · typescript BLOCK con 1 HIGH real** — `_guardar()` congelaba el payload **pero no la grilla**: tocar "✕ Quitar" durante los dos `await` borraba del bucket el path ya congelado en el snapshot y la fila quedaba con un `archivo_url` que no apunta a nada, en silencio (**caso simétrico** del guard de `_subiendo`). Más 2 MEDIUM: huérfanos al cambiar de cliente/cerrar el modal (**cuidado la invariante: tras guardar bien hay que vaciar `_retenciones`**, si no el `onClose` borra los archivos de la cobranza recién registrada) y `creditos-fiscales.js` asignando `_items` **antes** del chequeo de token → el CSV del contador podía no ser el período en pantalla. **Anotado y NO aplicado (decisión de Fede): `sql/storage_comprobantes_scope_finanzas.sql`** — el bucket da SELECT a **cualquier logueado**, y la RLS de `creditos_fiscales` protege **la fila, no el archivo**. Versiones: `api.js?v=97` · `cobranza.js?v=2` · `creditos-fiscales.js?v=2` · `finanzas.js?v=63`. Detalle: PROGRESO §[E2] 2026-07-31b.

- **Sesión 2026-07-30/31c (continuación autónoma — VENTAS FASE 2 Tasks 0-5 + push genérico + fix Eventos) — PUSHEADO (`ff995c7`·`5565375`·`4696de8`·`6de6ed6`·`f275601`) · SQL YA APLICADO POR MCP · ⏳ Fede: `~/pull-lobby.sh && cp /home/mepex/lobby/tools/vps/push.js /home/mepex/api/ && pm2 restart mepex-api`:**
  **(1) Eventos:** la sección "Fechas y horarios" tiene 4 inputs y escribía 11 campos → **colapsaba a un día los armados de varios y borraba la hora de cierre** que el trigger `fn_evento_jornadas_sync` había calculado desde `evento_jornadas` (la fuente de verdad). Afectaba a 5 de los 7 eventos vivos. `eventos.js?v=42`.
  **(2) 🔴 RLS de `notifications` cerrada** (`sql/notificaciones_rls_fix.sql`): tenía un `OR target_role IS NULL` suelto que dejó de significar "para todos" cuando el fan-out empezó a escribir avisos personales con `target_role` NULL → **cualquier logueado leía los avisos de toda la empresa**; UPDATE estaba en `USING(true)` (se podía reescribir el título de un aviso ajeno). El síntoma se había arreglado en el cliente el 30/07, la causa seguía abierta. **Barrido posterior: era el único caso.** De yapa apareció un **bug vivo**: el RETURNING de un INSERT lo filtra la policy de SELECT → un pm/taller que mandaba un **pedido de compra veía "No se pudo enviar" cada vez, con el pedido ya entregado**.
  **(3) Push genérico** `POST /api/push/aviso` — recibe la **referencia del suceso** y re-resuelve destinatarios server-side (el 1er diseño mandaba IDs de `notifications` y **dependía del agujero de RLS de arriba** para funcionar; el security-reviewer lo bloqueó con razón). Habilita el push de las filas 7/8/15 del Paso 9.
  **(4) VENTAS FASE 2 — Tasks 0 a 5.** `sql/ventas_fase2_creditos_fiscales.sql` **aplicado y verificado en prod con cleanup exacto**: 4 cuentas **1.1.11-1.1.14** (⚠️ el blueprint proponía `1.1.10`, que **está ocupado** por "Anticipos a proveedores"; `orden` 121-124, la convención es correlativa al código), tabla **`creditos_fiscales`** (retenciones y percepciones juntas: misma naturaleza contable, mismo libro), percepciones en `comprobantes_recibidos`, y la **línea de retención en `fn_asiento_auto_ingreso`** → **el asiento cuadra sobre lo DEVENGADO, no sobre lo que entró al banco**. Más `API.registrarCobranza` (el ingreso nace `pendiente`, después retenciones y aplicaciones, y **recién ahí** se confirma: es lo que hace que el trigger vea las retenciones), **`cobranza.js`** (el recibo, candado en vivo + re-lectura de saldos antes de guardar) y **`creditos-fiscales.js`** (el libro + export CSV). Verificado: cobro sin retención **idéntico al de antes** (inercia) · cobro 115.000 + retención 6.000 → cabecera 121.000 · multi-factura 1.000 con IVA → DEBE Caja 950 + Ret. 50 / HABER Ventas 826,45 + IVA 173,55.
  **REVIEWERS: 5 rondas, 1 CRITICAL + 9 HIGH, todos aplicados.** Lecciones que quedan como regla: **(a)** el trigger leía el IVA sólo de `ingresos.comprobante_id` y la cobranza multi-factura aplica contra `cobro_aplicaciones` → **el caso central de la fase habría posteado 100% a Ventas y CERO IVA, sin ningún NOTICE** (el total cuadraba, la composición no); **(b) `Number('1.000')` es 1**, y comparar dos importes mal parseados **entre sí** cuadra igual → `API._monto()` **rechaza lo ambiguo en vez de adivinar**; **(c)** una cobranza **100% retenida** pasaba sin `cuenta_id` y devolvía éxito **sin generar asiento**; **(d)** race que ataba la cobranza al **cliente equivocado** (sólo se deshabilitaba el botón, no el selector); **(e)** antes de mandar un `entidad_id`, mirar si esa tabla es **UUID o BIGINT** (`compras_ordenes.id` es bigint → el INSERT entero abortaba en silencio); **(f)** excluir al actor está bien en una acción humana y **mal en un barrido de fondo**; **(g)** un formulario **no escribe lo que no pregunta**; **(h)** `escAttr` no valida esquema de URL (usar `_safeUrl`) y un CSV para el contador necesita neutralizar `= + - @`.
  **Falta:** Task 6 (matriz de 13 escenarios por UI) + pulido visual de las 2 pantallas nuevas (con Fede mirando el render). Versiones: `api.js?v=96` · `eventos.js?v=42` · `finanzas.js?v=62` · `cobranza.js?v=1` · `creditos-fiscales.js?v=1`. Detalle: PROGRESO §[E2] 2026-07-30/31c.

- **Sesión 2026-07-30b (MATRIZ DEL PASO 9 — E7 / N5, cierra el frente de notificaciones) — PUSHEADO `2a88144` · SQL aplicado por MCP · ⏳ Fede: `~/pull-lobby.sh` + verificación logueada:** de las tres opciones del handoff, Fede eligió la matriz. **El reconocimiento redujo el alcance a la mitad y ahorró infra:** la matriz daba 15 sucesos por construir, pero `docs/handoff-capa-operativa-pulido-notificaciones.md` §FRENTE 3 documenta una tanda previa (2026-07-04) con los triggers de `sql/notif_operativas.sql` + **15 generadores en `alertas.js`** — filas 14/17 ya construidas, 12 ya existe como alerta. **Y cuatro filas que pedían "job diario" son ESTADO VIVO, no "pasó algo"**: como fila de `notifications` reaparecerían sin leer en cada recálculo (el argumento de la decisión D2 para no fusionar las pestañas de la campanita) → van como **alertas calculadas**, que ya se recalculan solas cada 5 min → **`pg_cron` NO se instaló** (disponible, sin usar). **Construido:** 7 tipos de aviso (`caso_nuevo`·`oc_generada`·`cotizacion_aprobada`·`evento_fecha_cambiada`·`evento_solapamiento`·`evento_armado_proximo`·`factura_emitida`) vía **`API.avisar`** (atajo nuevo sobre `resolverDestinatarios`+`notificar`), los 7 catalogados en el mismo commit + categoría nueva **Finanzas**; **3 alertas** (tareas vencidas · vencen hoy · **cuotas de cobranza vencidas**, generador `finanzas` nuevo — ya tiene data real: 1 cuota, $4M sin cobrar). **Fila 7** (armado a 7/2 días) usa el **claim atómico** de `notifyPagosVencidos` sobre 2 columnas nuevas de `eventos` (`sql/notif_matriz_paso9.sql`, aditivo + backfill de armados pasados para no disparar retroactivos); caveat asumido: si nadie abre la app en el día, sale al siguiente. **Fila 15: la matriz la daba por cubierta y no lo estaba** — `eventos.js _detectConflicts` busca personas doble-asignadas, no armados superpuestos, y solo con el caché cargado → se consulta a la base al cambiar la fecha. **Reviewers: security APPROVE · typescript BLOCK con 3 HIGH, los tres reales.** Lecciones que quedan como regla: **(a) `compras_ordenes.id` es BIGINT y `notifications.entidad_id` es UUID** → mandarlo abortaba el INSERT entero y el aviso de OC se perdía **el 100% de las veces sin ruido** (y `notifyPagosVencidos` ya documentaba el mismo choque al lado); **(b) excluir al actor está bien en una acción humana y MAL en un barrido de fondo** — ahí el logueado es solo el que tenía la pestaña abierta, y excluirlo le come el aviso de su propio evento con el claim ya consumido (flag `excluirActor`); **(c) `eventos.js:2778` manda siempre `setupEndDate = setupDate`** → `fecha_armado_fin` se reescribe en cada guardado, así que vigilarla daba un falso "cambió el fin del armado" en todo evento multi-día (se vigilan solo las fechas de inicio; **el bug de datos de fondo queda pendiente: un armado de 3 días se vuelve de 1 sin avisar**); (d) `resolverDestinatarios` matchea el rol **literal**, sin jerarquía → `superadmin` hay que nombrarlo. **Cabos:** el push **no aplica** a estos avisos (el único camino es `/api/push/tarea`, que re-resuelve destinatarios a partir de una tarea) → filas 7/8/15 quedaron solo campanita y no se puso un flag `push` que no hiciera nada; filas 9 y 16 afuera (piden estado de evento y tabla `ausencias`); **la RLS de `eventos` es `USING(true)` para cualquier autenticado** (previo, desentona con la matriz `fn_role_can`). `api.js?v=94`·`alertas.js?v=9`·`notifications.js?v=12`·`finanzas.js?v=60`. Detalle: PROGRESO §[E2] 2026-07-30b.

- **Sesión 2026-07-29/30 (TAREAS v2 + WEB PUSH VAPID + REWORK DE NOTIFICACIONES) — PUSHEADO · ✅ PUSH ANDANDO EN PROD:** ejecución de los dos docs de JordiGPT (`docs/jordi/01-*` y `02-*`). **Planes que mandan: `docs/jordi/03-PLAN-EJECUCION-TAREAS-PUSH.md`** (8 etapas, D1-D6) y **`docs/notificaciones-rework-plan.md`** (N0-N5). **🔴 Hallazgo más grave: la RLS de `tareas` estaba ABIERTA** (`USING(true)` de fase11) → cualquier logueado leía/borraba TODAS las tareas desde la consola; cerrado con `sql/tareas_v2_asignados_rls.sql` + `sql/tareas_v2_fix_rol_check.sql` (corridos). **Construido:** E1 asignación múltiple (`tarea_asignados` N roles + N personas) + historial inmutable (`tarea_actividad`) + `fn_tarea_visible()` · E2 fan-out único (`API.resolverDestinatarios`/`notificar`, dedup + excluye creador) · E3 **Kanban conviviendo con la Lista** (D1: la Lista NO se saca, es la bandeja del taller en el celular) · E4 **la app ahora es PWA** (`manifest.json` + `sw.js` + iconos; **el SW no cachea nada a propósito** — sin handler de `fetch` no puede romper la carga) · E5/E6 push VAPID (`tools/vps/push.js`, `push-cliente.js`, `sql/push_subscriptions.sql`; `/api/push/tarea` re-resuelve destinatarios e `is_urgent` server-side) · E8 `docs/guia-instalar-app-celular.md`. **Rework de notificaciones N0-N4:** catálogo corregido (18 tipos / 8 categorías) + `notificacion_prefs` (**matriz categoría × canal por usuario, aplicada a prod**) + el push respeta la columna Celular + horario de silencio 21-07 + pantalla unificada con "Tus dispositivos" + campanita alineada ("Avisos"/"Pendientes"). **⚠️ LECCIONES CARAS:** (a) **`server.js` requiere 6 módulos hermanos** — copiarlo sin ellos tiró `mepex-api` a crash-loop y se llevó puestos ARCA/CRM/OCR (faltaba `whatsapp-webhook.js`); **verificar los `require()` antes de mandar un `cp`**; (b) **una service key que no saltea RLS falla EN SILENCIO** (lee 0 filas, sin error) → por eso `push.js` ahora hace un `autodiagnostico()` al arrancar; las legacy `eyJ…` están deshabilitadas por Supabase desde 2026-04-06, van `sb_secret_`; (c) **la auditoría por grep NO ve los emisores de notificación que viven en triggers de Postgres** (hay 3) — casi borro una categoría creyéndola fantasma; (d) **`Tareas._esc()` (textContent→innerHTML) NO escapa comillas** → dentro de atributos va siempre el `escHtml`/`escAttr` global de components.js; (e) **texto de UI que promete algo que el código no hace = el mismo bug que un switch que no silencia nada** — si no se puede cumplir, se construye o no se escribe; (f) **zonas horarias en el VPS: offset fijo, NO `Intl`** — Argentina es UTC-3 siempre, y `Intl` con `timeZone` **falla callado** en un Node con small-icu (devuelve UTC). **🐞 Última ronda (`44e7c0c`), 3 bugs que cazó Fede probando:** (1) **todos veían las notificaciones de todos** — `getNotifications` trataba `target_role IS NULL` como "para todos", y el fan-out escribe cada aviso personal con ese campo en null; el broadcast ahora exige `target_user_id` **y** `target_role` en null; (2) **el avance notificaba a los 7 superadmins** (el doc de Jordi asume que el super_admin es UNA persona; acá son 7) → ahora avisa solo a **quien creó la tarea**; (3) el **auto-tagueo explícito** ahora sí llega. **⏳ Deploy pendiente:** `cp tools/vps/push.js` al VPS + `pm2 restart mepex-api`. **Handoff: `docs/handoff-tareas-notificaciones.md`.** Detalle completo: PROGRESO §[E2] 2026-07-29/30.

- **Sesión 2026-07-28/29 (CIRCUITO DE VENTA — Fase 1) — PUSHEADO + ✅ VERIFICADO EN PROD:** nació de la charla de Fede con Sofi sobre retenciones. El pedido era "falta el módulo Ventas"; en realidad eran 3 agujeros: **emitir factura NO genera asiento** (contabilidad de ventas por percibido → sin Deudores vivo, IVA desfasado vs DDJJ, y **la retención sin dónde caer**), la venta repartida en 5 tablas sin un lugar donde verse, y el canal interno sin documento propio. **Spec obligatoria: `docs/circuito-venta-blueprint.md`** — 13 decisiones con su porqué, modelo de datos, asientos escritos, reglas (derivados/candados/semáforo), contrato de no-regresión, matriz de simulación, y **§12.bis con la verificación contra prod**. 4 fases: **1 (la venta existe) ✅ HECHA** → 2 (retenciones + recibo de cobranza, lo de Sofi) → 4 (contrato con firma) → 3 (devengado, la brava). **Construido:** `ventas.js?v=1` (listado) · `venta-detalle.js?v=1` (ficha, `renderInto` = página y modal) · `api.js?v=91` · `crm.js?v=41` (ganado **propone**, no crea; el proyecto nace en `negociacion`) · `finanzas.js?v=58` · `data.js?v=28`/`router.js?v=24` **en index.html (son CORE)** · `sql/ventas_fase1.sql` + `sql/ventas_fase1b_plan_cobro_nullable.sql`, ambos corridos. **Verificado en prod con cleanup exacto** (partida doble idéntica, $18.984.910). **✅ El plan de pagos se arma DESDE la ficha de la venta** (`venta-detalle.js?v=2`/`finanzas.js?v=59` + `sql/ventas_fase1c_plan_unico_por_venta.sql`): atajos 50-50 / 30-40-30 / 40-30-30, suma en vivo, agregar cuota (= el adicional de último momento). 4 reviewers cazaron 3 HIGH, los tres de **divergencia silenciosa entre lo que se ve y lo que se guarda** — el peor: tocar el campo Total recalculaba desde un % redondeado y **movía montos tipeados a mano mientras el pie decía "✓ cuadra"**. **Lecciones caras que quedan como regla:** (a) `CREATE FUNCTION` concede `EXECUTE` a **PUBLIC ⊃ anon** → toda RPC nueva necesita `REVOKE ALL ... FROM PUBLIC, anon` + guard de rol adentro, si no queda expuesta por PostgREST con la anon key pública; (b) `const { data } = await supabaseClient...` sin mirar `error` convierte un timeout en "cero filas" — en un guard fiscal eso **falla abierto**; (c) el RLS de este repo va por **matriz `fn_role_can(modulo, need)`**, y `roles.permissions` es un **objeto** `{"mod":"write"}` con PK `roles.id` (no array, no `roles.nombre`) — el grant del módulo y el RLS son **el mismo paso**; (d) **`_configVersion`/`sidebar-editor.js` ya no existen** — `app.js:167` arma el sidebar directo de `Data.categories`, así que la nota de §10 (2026-05-30) sobre bumpear `_configVersion` quedó VIEJA. Detalle completo: PROGRESO §[E2] 2026-07-28/29.

- **Sesión 2026-07-27 (Undo/Redo arreglado de raíz + cobertura ~40 flujos + planes de cobro verificados) — PUSHEADO · ⏳ Fede: `~/pull-lobby.sh`:** reporte de Fede ("borré una cuota y Ctrl+Z no anduvo; no pude crear un plan"). Verificado en prod: crear plan + agregar cuota FUNCIONAN (su cuota borrada fue **restaurada**); el undo no andaba porque **`Confirm.delete()` prometía Ctrl+Z con texto fijo que mentía en 18 de 22 lugares** (solo 10 de ~52 flujos registraban undo). Batch 13 archivos: `undo.js?v=4` (error-checks en undo/redo — chau "Deshecho ✓" falso — + `_refreshView` vía Router para módulos custom + UndoUI idempotente) · `app.js?v=16` (Ctrl+Z ya no se intercepta tipeando en inputs + dedupe del keydown en re-login) · `auth.js?v=8` (clear() del stack en logout) · `components.js?v=10` (Confirm honesto `{undoable}` + escHtml) · `api.js?v=88` (15 deletes → UndoHelpers + **compuestos deletePlanCobro/deleteTransporte** — el primero arregla cuotas huérfanas) · inline convertidos en finanzas v56 / contabilidad v17 / compras v20 / rrhh v17 / tareas v12 / locaciones v12 / calendario-adm v3 / stands v5. **Ingresos/egresos SIN undo a propósito** (triggers contables — diálogo dice la verdad con `undoable:false`). Reviewer typescript: APPROVE 0 C/H (2 MEDIUM aplicados). **+ 2ª tanda (`e264f98`, `finanzas.js?v=57`): Fede cazó en prod "Error al guardar: invalid input syntax for type uuid: undefined" al COBRAR una cuota → root cause: `_showIngresoModal(prefill)` con `isEdit = !!ingreso` — los prefills SIN id (Cobrar cuota, Duplicar ingreso, Duplicar egreso) caían en modo Editar y el UPDATE iba con `id="undefined"`. Fix: `isEdit = !!(ingreso && ingreso.id)` en ambos modales (los 3 flujos estaban rotos).** **+ Barrido de circuitos Finanzas por plomería (prod vía Chrome, datos fake + cleanup EXACTO, partida doble verificada cuadrada $13.984.910 al cierre):** ✓ cobro→asiento balanceado→anular→contra-asiento ✓ gasto pagado→asiento ✓ e-cheq→cartera+asiento 1.1.07 ✓ comprobante recibido→generar pago→asiento 3 líneas con IVA crédito ✓ transferencia→asiento auto ✓ cobro de cuota con sync (monto_cobrado+estado) — TODO el backbone sano; los bugs eran solo de modal. ⏳ Fede: re-pull (v57) + probar Cobrar cuota y Duplicar por UI. Pendientes de la próxima pasada undo + detalle completo: PROGRESO §[E2] 2026-07-27.

- **Sesión 2026-07-26 (MFA lockout + Security Advisor Supabase — TODO RESUELTO Y VERIFICADO EN PROD, SQL-only):** Fede lockeado del lobby (el TOTP rebotaba en incógnito+celu; recon: GoTrue sano, sin incidentes, `auth.js` intacto → no era código; 1 solo factor, sin duplicados). **Rescate MFA estándar** (documentado en PROGRESO §[E2] 2026-07-26): SQL Editor → `DELETE FROM auth.mfa_factors` del usuario → login solo-password → re-enroll limpio en Mi Perfil ✅. **Advisor:** el CRITICAL `rls_disabled_in_public` (mail 20/07) se cerró con **`sql/seguridad_rls_advisor_fix.sql`** (descubrimiento dinámico `pg_tables NOT rowsecurity` + purga policies anon/public huérfanas + enable + authenticated-all; el sql-reviewer cazó 2 HIGH antes de correr) y el rerun destapó lo grave: **las 5 views financieras (`v_plan_cobro_resumen`/`v_libro_iva_compras_extendido`/`v_posicion_iva_mes`/`v_saldo_comprobante`/`v_saldos_apertura_ejercicio`) le devolvían data real a cualquier anónimo** (las views bypassean RLS por default) → **`sql/seguridad_views_invoker_fix.sql`** (`security_invoker=true` ×5 + REVOKE anon; reviewer APPROVE). **Linter: 5 errors → 0.** Verificado triple: anon 401×5 · cotizador intacto (226 ítems) · sesión admin idéntica a antes (5/2/3/2/24 filas, UI Finanzas/Contabilidad OK, 0 errores consola). **⚠️ Regla de facto nueva: toda view nueva nace con `security_invoker = true`.** Backlog anotado en PROGRESO: re-etiquetar 4 policies `{public}`→authenticated (hoy inofensivas) · 182 warnings del linter · opcional acotar el anon de `catalogo_items`. Sin JS, sin bumps.

- **Sesión 2026-07-22b (REVISADA INTEGRAL del plan — verificación total contra prod + PLAN-SUPERIOR reescrito de cero) — PUSHEADO:** pedido de Fede "revisá bien todo el plan, lo completado a PROGRESO, visualicemos lo pendiente". **Auditoría de deudas fantasma: TODO corrido** — SQLs (`stands_predisenos` · `eventos_link_organizador` · `eventos_jornal_sync` · `proyecto_conformes` · `catalogo_showroom_f1` · `fase7 saldos_apertura` [pausa 2027]) + los 5 buckets (`proyecto-fotos`/`stands`/`comprobantes`/`catalogo`/`remitos` — stands y comprobantes CON contenido = en uso; download anon bloqueado) + RLS docs/historial activa (memoria `project_fase4_rls_docs_historial` estaba vieja) + UI jornal RRHH = falso pendiente (campo "Jornal diario" = `costo_dia_referencial`, ya en Nómina). **Lección de método Storage:** el probe `public/<bucket>` da "Bucket not found" para privados EXISTENTES (falso negativo) → usar `POST object/list/<bucket>` (y upload-probe anon: 415 mime = existe). **PLAN-SUPERIOR reescrito:** podado todo lo ✅ (cada ítem verificado en PROGRESO antes por grep), sección "Verificación integral" anti-re-duda, compromisos nuevos de Fede (celu WhatsApp SÍ O SÍ semana 27/07 · iPlan lunes 28/martes 29 desde casa · ronda sugerida 03/08), explicados E4 fase 2 y Marketing (Fede no los tenía claros). Solo SQLs sin correr = los destructivos diferidos A PROPÓSITO (DROPs legacy, reorg_cleanup). Detalle: PROGRESO §[E2] 2026-07-22b.

- **Sesión 2026-07-22 (Peinada PLAN-SUPERIOR + verificación prod + barrido botón-volver ✅) — PUSHEADO · CERO pendientes de pull/SQL de Fede al cierre:** sesión "cierre y detalles" (ventana Jordi). **(1) Prod verificado por curl:** v40/v87 servidas (pull 4ª tanda ✓), `posthog-init.js?v=2` (Error Tracking ✓), **SQL `wa_eventos` YA en prod**, webhook WA 404 = falta solo el deploy VPS del runbook (paso de la sesión con celu — todo listo para cuando Fede traiga el celu). **(2) PLAN-SUPERIOR peinado** (consolidación 2026-07-22: descontado lo del 18-19/07, orden recomendado al día, fin piloto claude-mem ~25/07). **(3) Barrido global botón-volver** (pendiente conexo de la ficha v4): clase global **`.btn-back-circ`** en `style.css?v=18` (calco de `.caso-back-v3`) → 8 back de texto convertidos (`finanzas.js?v=55` ×2 · `inventario.js?v=20` · `locaciones.js?v=11` · `stands.js?v=4` · `compras.js?v=19` ×2 · `importar-contactos.js?v=2` · `importar-cotizacion.js?v=2`) + `.ev-panel-close` de Eventos alineado (solo CSS, sin bump de eventos.js) + 5 clases CSS huérfanas borradas. NO tocados: wizards "← Atrás", prev/next catálogo, nav calendario, breadcrumb proyecto-detalle. IDs/handlers intactos. Verificado (node --check + preview computed styles + headers montados) + reviewer typescript **Approve 0 C/H** (LOW: 30px < 44px táctil mobile, mismo lote que `.caso-back-v3`). `RESPUESTAS-JORDI.md` (cuestionario pre-llamada) quedó untracked en raíz, es de Fede. Detalle completo: PROGRESO §[E2] 2026-07-22 (+ registrada la entrada faltante [E2] 2026-07-19c de la ficha v4).

- **Sesión 2026-07-19 (CRM Ficha v4 — flujo de mensajes DEFINITIVO, plan-first cumplido) — PUSHEADO (`14d444b`) + VERIFICADO EN PREVIEW · ⏳ Fede: pull + cp connector:** ejecución del handoff `docs/handoff-crm-ficha-flujo.md`. **Plan-first:** diagrama del flujo (2 carriles + 3 opciones A/B/C) + 2 mockups full-screen fieles → **Fede eligió M2 "Copiloto"** con podas (afuera del copiloto: Mandar COT / Agendar seguimiento / Crear tarea; queda sugerencia + Redactar + 48h) y pidió: header descomprimido, burbujas más compactas, Drive con botón, countdown simple, frases hechas, **diseño DEFINITIVO asumiendo ingesta automática** (E4 lo resuelve la semana del 20/07) con **registro manual delicado, nunca protagonista**. **Construido (`crm.js?v=37` · `api.js?v=86` · bumps en `App._APP_SCRIPTS`):** (1) **Header v3.1** — subtítulo afuera (tooltip), derecha 2 filas (acciones / sin-mov·edad·owner), izquierda cliente+estado+temp / evento+línea. (2) **Hilo compacto** — agrupado por lado (`tl-tight`), hora inline mono adentro de la burbuja, paddings finos. (3) **Composer v4 = MANDAR** — barra única (＋ | textarea autogrow | chip destino WhatsApp/Nota que tiñe | enviar); Enter manda; WhatsApp = mensaje saliente al hilo + abre `wa.me` con el texto (**post-E4 la Cloud API se enchufa acá, sin rediseño**); frases hechas editables (chips → prearman, localStorage `mepex_crm_frases_v1`, tokens `{nombre}`/`{cot}`); menú ＋ = foto / link COT PDF / link Drive / **"Algo pasó afuera" al fondo** → modal con el registro completo viejo (canal+dirección+Procesar IA). (4) **Resumen IA AUTOMÁTICO** al abrir/recargar si hay mensajes nuevos (lock POR caso, `_iaAutoFailed` anti-martilleo que el ↻ manual resetea, swap solo de la banda preservando el desplegable). (5) **Copiloto** (aside) — sugerencia heurística client-side, **"Redactar respuesta"** (nuevo `mode:'redactar_respuesta'` en `tools/vps/crm-digest.js`, borrador al composer, JAMÁS auto-envía), **toggle 48h** (reusa `proximaAccion` con marker — cero DDL, alimenta Bandeja), **Evento countdown** (`API.getEventoFechas` lazy). **Reviewers regla 19 (typescript + security): 4 HIGH arreglados** — 3 races post-await (redactar a composer de otro caso · `_reloadCasoYFicha` pisaba mensajes con id viejo · lock global del auto-resumen) + **XSS en `Modal.confirm` de `_toggle48h`** (`proximaAccion` sin escapar) + MEDIUMs (adjuntos del modal al target correcto, spinner del Procesar, prompts del connector con guard anti prompt-injection). Verificado por `preview_eval` (render completo + interacciones + los 6 fixes, 0 errores). **SIN SQL.** **✅ v37/v86 VERIFICADO EN PROD** (Fede pulleó; testeado vía su Chrome con caso temp + cleanup 2026-07-19): auto-resumen IA real anduvo solo ("auto · hace 1 min", digest 200 provider claude), nota+WhatsApp al hilo (wa.me con 54 OK), frases, 48h strip, modal registro con llamada — 0 errores consola. "Redactar" toast esperado (connector viejo en VPS). **+ 2ª tanda misma sesión (`crm.js?v=38`/`api.js?v=87`): RESUMEN INCREMENTAL como LEY** (regla de Fede: "lo que está hecho ya está hecho") — `generarResumenCaso` manda SOLO mensajes nuevos desde `resumen_ia_at` + el resumen actual con **mode `resumen_caso_inc`** (suma frases, tope ~8, comprime lo viejo); sin nuevos = CERO llamada; delta >30 nuevos = cae a full (evita hueco permanente, hallazgo MEDIUM del reviewer); guard anti-clobber (inc solo acepta `resumen_caso` → connector viejo NO pisa el resumen bueno); "**Rehacer de cero**" = botón discreto al pie del desplegable (única re-lectura total). ↻ manual también incremental + toast "ya está al día". **+ Inventario IA afinado: `docs/ia-inventario.md`** (fuente de verdad del control de modelos/créditos): 3 motores lobby (digest/OCR/OCTEXA) + cotizador = **todo Claude Haiku 4.5**, cambiar de modelo = 1 env + restart, escalera Sonnet/Opus documentada; los 3 endpoints ahora **devuelven `model` en el JSON** (control a ojo en F12). Reviewer 2ª tanda: Approve 0 CRITICAL/HIGH. Verificado en preview (delta grande→full · chico→inc · sin nuevos→sin llamada · primer→full). **⏳ Fede (ORDEN VPS-FIRST, como SQL-first): 1º `cp /home/mepex/lobby/tools/vps/{crm-digest,ocr-comprobante,octexa-ia}.js /home/mepex/api/ && pm2 restart mepex-api` → 2º `~/pull-lobby.sh`** (al revés, el auto-resumen tira error hasta el cp). **+ 3ª tanda misma sesión (`crm.js?v=39`, `296a9d3`):** Fede cazó en prod el toast engañoso del Copiloto en un caso SIN mensajes ("falta connector" cuando el problema era que no había conversación) → botón Redactar disabled con tooltip + toast honesto; sonda en prod confirmó que su cp de la 1ª tanda YA hace andar "Redactar" (borrador real provider claude). **+ 4ª tanda — pulido v4.1 (`crm.js?v=40`, `2799707`):** pedidos de Fede antes de cerrar: (1) **"Respuestas frecuentes"** (rename de frases hechas) con botón ▴ que las despliega ENTERAS arriba del composer (elegir inserta y cierra) + editor por filas (input+✕+agregar, tope 8); (2) **pipeline sin importes** y cards al idioma Bandeja (avatar, cliente protagonista, chips urgencia, COT chip, owner; DnD intacto); montos también afuera de la ficha de cliente y del panel (regla: importes SOLO en el presupuesto); (3) **todo igualito**: ficha de cliente con el header de la ficha de caso (back CIRCULAR — chau "← Clientes" — avatar color, chips, nums a la derecha) y cierre de paneles laterales circular. Reviewers: security Approve (0 C/H, +2 escapes preexistentes tapados) · typescript cazó **1 HIGH real**: `_enrichCasos()` filtraba ganado/perdido → columnas Ganado/Perdido del Kanban vacías y DnD que "desaparecía" la card; fix `_enrichCasos(todos=true)` solo pipeline (Bandeja intacta) + reset `_frasesOpen` por caso. Verificado en preview (30+ checks, 0 errores). MEDIUM anotado no bloqueante: CSS muerto (.crm-panel-list-monto/.caso-row-owner/.caso-pcard-titulo/.caso-pcard-cli) para barrido futuro. **⏳ Fede: `~/pull-lobby.sh`** (v40). **Pendiente conexo:** E4 celu (semana 20/07) enchufa el envío real; barrido botón-volver global (fuera de CRM) sigue en cola.
...
- **Sesión 2026-07-17 (Consolidación PLAN-SUPERIOR + WhatsApp prep + plugins + PostHog) — TODO PUSHEADO Y VERIFICADO:** seguridad cerró **40/40** (MFA activo en Fede). **WhatsApp E4 listo para la sesión con celu** (agendada semana del 20/07 mar/mié): webhook Coexistence con firma HMAC + staging `wa_eventos` + runbook `docs/whatsapp-coexistence-runbook.md`. **PLAN-SUPERIOR consolidado** (5 frentes, +Consultoría Jordi; 4 SQLs "pendientes" verificados YA aplicados en prod). **`docs/jordi/`** creada con el prompt para aplicar los 4 MD de Jordi (sesión aparte). **3 plugins instalados por CLI** (supabase / superpowers / claude-mem — ⚠️ revisar solape claude-mem vs `memory/`). **PostHog Analytics EN PROD** (proyecto 518740 US): `posthog-init.js` post-login como script **OPCIONAL** del loader (`App._OPTIONAL_SCRIPTS` — analytics jamás bloquea la app), identify usuario+rol, pageviews por hash con prop `modulo`, `capture_exceptions`; CSP con hosts posthog. Verificado en prod: `distinct_id: fede`. Ronda de testeo: pendiente sin fecha (¿27/07 o agosto?).
