...
> **Reorg capa operativa física (Fases A→F) = additivo 100% construido y pusheado.** Tracking canónico en `PROGRESO.md` §sesiones 2026-06-24/25 + `PLAN-MAESTRO §REORGANIZACIÓN DE LA CAPA OPERATIVA FÍSICA` + handoff `docs/reorg-capa-operativa-handoff.md`. Spec: `docs/capa-operativa-blueprint.md`. **Restan SOLO las 2 piezas destructivas** (borrar `taller.js` + desconectar `logistica.js` → requieren OK explícito de Fede).
