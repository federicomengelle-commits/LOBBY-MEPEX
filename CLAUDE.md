...
- **Sesión 2026-06-21 (charla Finanzas — continuación 2) — verificación de 3d.2/Opción A + Fase 4 v1.1 (KPI + cheque/e-cheq en modales) · TODO VERIFICADO EN PROD (Chrome, con cleanup)**:
  ...
  - **⏸️ Fase 4 v1.1 piece 3 (DEFERIDO a propósito) — saldo por cuenta vía `saldos_mensuales`:** hoy el "Saldo disponible" suma `ingresos`/`egresos` por `cuenta_id` (`finanzas.js:5840`, comentario "Fase 12.D mejora futura"). El cheque emitido tiene `cuenta_id`=banco → lo descuenta del saldo ANTES del débito real (el recibido no infla porque va con `cuenta_id=null`). El fix correcto es leer el saldo contable materializado (la pata va a `2.1.07`/`1.1.07`, no al banco). **Pero cambia un número que Fede reconcilia contra el Balance (caso histórico $8.75M Oficial vs $6.5M) → pasada dedicada SQL-first con su ojo en los números, NO a ciegas de noche.**
  - **⏳ RESTA (orden por valor):** Fase 4 v1.1 piece 3 (saldo vía `saldos_mensuales`) · **3b.2** (switch `compras.js`→proveedor UUID, all-or-nothing, dedicada) · Fase 5 (conciliación CSV Galicia/MercadoPago + borrar `compras_pagos` muerto + solape calendario finanzas↔calendario-adm) · **Fase 6 ARCA** (👉 bloqueada por trámite del certificado X.509 — arrancarlo YA; "Emitir"=botón SDK ARCA reemplaza La PyME greenfield, PERO la carga manual de emitidos SE QUEDA para informal-foto/factura E/factura M) · Fase 7 (cierre pre-2027: mapeo_cuentas CRUD + auditoría integridad + saldos apertura + bloqueo ejercicio). Plan que MANDA: `docs/finanzas-contabilidad-refactor-PLAN-EJECUCION.md`.
