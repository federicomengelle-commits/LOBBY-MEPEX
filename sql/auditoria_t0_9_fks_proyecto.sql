...
--   plan_cobro con proyecto_id inexistente : 1  ⛔ ver abajo, queda AFUERA
...
--
-- =====================================================================
-- ⛔ `plan_cobro` NO ENTRA EN ESTE ARCHIVO — y el motivo es una trampa
-- =====================================================================
...
--
-- Salir de ahí pide **borrar físicamente esa fila** (cascadea a sus
-- `plan_cobro_items`), y eso es destructivo → **decisión de Fede**.
-- Hasta entonces `plan_cobro` se queda sin FK. Anotado como T0.9b.
-- =====================================================================
...
BEGIN
    ...
    RAISE NOTICE 'T0.9 OK — 0 huérfanos, 2 FKs creadas. plan_cobro queda pendiente (ver cabecera).';
END $$;

COMMIT;

-- =====================================================================
-- T0.9b — lo que queda abierto en este frente
-- =====================================================================
-- 1. `plan_cobro`: la FK, después de resolver la fila 8c2b86c9 (ver arriba).
-- 2. Hay **5 tablas más con `proyecto_id UUID` y ninguna FK**, verificado
