...
| **H7** ⏳ | **ABIERTO — y al diagnosticarlo el 20/8 resultó más preciso y más grave de lo que decía esta fila.** **No es que "falte una columna": la cuenta corriente YA está bien resuelta.** La view `v_saldo_comprobante` aplica `fn_signo_comprobante`, que devuelve **−1** para todo `nota_credito%`, así que una NC entra con **total y saldo negativos** y su `estado_cobranza` es `'nota_credito'`. Sumado por cliente, **la NC descuenta bien**. **El bug está en el filtro de la pantalla de cobranza:** `API.getSaldosComprobantesPorCliente` trae con `.gt('saldo', 0.01)`, y como el saldo de una NC es **negativo**, la NC **nunca aparece en la grilla**. Efecto concreto: un cliente con una factura de $7.260.000 y una NC del mismo importe que la anula muestra **la factura entera como cobrable y la NC en ningún lado** — el operador puede aplicarle un cobro a una factura anulada. **Dos caminos:** (a) mostrar también las NC en la grilla, con su signo, para que se apliquen como cualquier otro movimiento — es lo coherente con el modelo, que ya piensa en cuenta corriente; (b) atar cada NC a su factura y netear, que es más trabajo y más rígido. **Recomendado (a).** ⚠️ **No se aplicó de madrugada a propósito:** toca el flujo de cobranza, que es lo más sensible del sistema, y se prueba escribiendo en la contabilidad de producción → **va con Fede mirando, en el mismo rato que la matriz de 13 escenarios de §C5** |
