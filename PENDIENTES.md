...
- **El aviso de "hay que recalcular" vive sólo en el panel abierto y se pierde al cerrarlo** — es *la* causa de que el ítem 89 quedara $40.240 desfasado sin que nadie lo viera. El arreglo es un contador persistente de "N ítems desfasados" en el módulo Costos, o un badge por ítem en la tabla. **Hoy el contador arrancaría en 1** (sólo el 89, congelado a propósito), así que es el mejor momento para ponerlo: nace limpio y a partir de ahí cualquier desfase nuevo se ve. Es pulido de UI → va con Fede mirando el render (skill `pulir-pantallas`).
- **Sacar de la pantalla de Parámetros los 6 que la fórmula ignora** (`hora_montajista_ars`, `vida_util_default`, `pct_desperdicio_aluminio`, `pct_indirectos_comercial`, `pct_reacondicionamiento`, `dolar_bna_vendedor`). Están a la vista invitando a que alguien los cambie creyendo que hacen algo.
- **Dos convenciones de porcentaje conviviendo** en el mismo modelo: los tipos de amortización guardan `15` = 15%, los parámetros globales guardan `0.30` = 30%. Cargar un desperdicio como `0.15` da 0,15%; un margen como `50` daría 5000%. Nada lo avisa.
