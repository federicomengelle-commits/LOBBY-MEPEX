...
| **H7** ⏳ | **ABIERTO — re-diagnosticado el 2026-08-23, y el camino recomendado CAMBIA.** Lo que decía antes sigue siendo cierto: la view `v_saldo_comprobante` aplica `fn_signo_comprobante` (−1 para toda NC), así que **sumado por cliente la NC descuenta bien**, pero `API.getSaldosComprobantesPorCliente` filtra `.gt('saldo', 0.01)` y **la NC nunca aparece en la grilla de cobranza** ([api.js:7064](api.js:7064)). **Lo nuevo, y es lo que decide el diseño: el agujero no es de visualización.** Aunque se mostrara la NC, **aplicarle un cobro no significa nada** —una NC no se cobra, descuenta— y sobre todo: **el candado de guardado compara factura por factura, no el neto del cliente** ([cobranza.js:682-690](cobranza.js:682)). Re-lee con `soloPendientes:false`, o sea que **sí ve la NC**, pero sólo chequea `aplicado <= saldo de esa factura` → una factura anulada por una NC **sigue teniendo su saldo intacto** y el candado la deja cobrar. O sea que la opción (a) del handoff **no cierra el agujero**. **★ Y el hallazgo que abarata la opción correcta: el sistema YA SABE qué factura anula cada NC, y tira el dato.** El wizard de emisión pide el comprobante asociado y **es obligatorio** ([finanzas.js:7430](finanzas.js:7430) y la validación en `:7937`), lo resuelve a un UUID local (`d.cbte_asoc_id`), lo usa para armar el `CbtesAsoc` que va a AFIP… y **`_buildComprobanteRecord` no lo guarda** ([finanzas.js:8458](finanzas.js:8458)). Verificado en prod: el `lapyme_response` de la única NC **no tiene `CbtesAsoc`** por ningún lado. **Entonces netear —que este documento descartaba por «más trabajo y más rígido»— es en realidad lo más barato: una columna `comprobante_asociado_id`, una línea en `_buildComprobanteRecord`, y el backfill de UNA sola fila** (la NC B 00005-00000002 del 22/6, $1.000, contra la factura B del mismo número y cliente del día anterior). No hay que pedirle nada nuevo al usuario ni cambiar el flujo: el dato ya está en la mano y validado. Con el vínculo guardado, la view netea sola y el candado puede saber que la factura está anulada. ⚠️ **Sigue yendo con Fede mirando:** es DDL + money-moving code + toca el flujo de cobranza, y se prueba escribiendo en la contabilidad de producción. Va junto con la matriz de 13 escenarios de §C5. **Dato de paso:** una factura B y su nota de crédito B **comparten el `numero`** (`00005-00000002` las dos) — es correcto en AFIP porque son tipos distintos con numeración propia, pero significa que **`numero` solo no identifica un comprobante**: hace falta `tipo` + `numero` |
