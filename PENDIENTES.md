...
| **7** | **`~/pull-lobby.sh` en el VPS** — hay código nuevo sin servir | 1 minuto | Prod está sirviendo `api.js?v=110` / `costos.js?v=39`; el repo va en **111 / 40**. Sin el pull, **el chip de precios desactualizados no existe** para nadie. Es lo único de hoy que necesita deploy — el SQL ya está aplicado |
...
- **Pulido visual del chip `⚠ N desactualizados`** — construido y verificado por datos el 6/8, pero **nadie lo vio renderizado**: el módulo Costos necesita sesión y el preview local no la tiene. Va con Fede mirando, después del pull.
