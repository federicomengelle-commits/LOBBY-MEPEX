...
> **🚧 ESTADO (2026-06-20, sesión dedicada EN CURSO):** Fase 0 recon vivo ✅ · Fase 1 P0 comprobantes ✅ (pusheado `9e75825`) · blast-radius 107 paths ✅ (`docs/finanzas-blast-radius-map.md`) · **Fase 2 SQL listo, ⏳ Fede corre** (`sql/fase2_iva_y_ingreso_asiento.sql`: IVA + asiento de ingreso + criterio anticipos `2.1.06`). Lo hecho → PROGRESO §sesión Finanzas. **🧭 El plan que manda: `docs/finanzas-contabilidad-refactor-PLAN-EJECUCION.md` (7 fases).** Restan Fases 3-7: circuito único `registrar_gasto` + proveedores UUID · tesorería + cartera de valores · conciliación CSV · ARCA nativo (greenfield, bloqueado por cert) · cierre 2027. El texto de abajo es el alcance/contexto original — el plan-ejecución lo supersede.
