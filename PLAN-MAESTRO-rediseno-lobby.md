...
- **RRHH.5 — Jornales** ✅ DESBLOQUEADA — el contrato `API.getJornalesByPersona(personaId)` ya existe (Rendimiento, 2026-06-18). Falta solo construir el tab lente-persona read-only en RRHH.
...
- **✅ "Rendimiento por evento" → MÓDULO CONSTRUIDO (2026-06-18, REND.1→REND.5 en una pasada — ver PROGRESO §Fase 8).** Pendiente solo: Fede corre `sql/rendimiento_evento.sql` + verifica GATE A/B + pull + test end-to-end. Spec madre `docs/modulo-rendimiento-evento-blueprint.md`. **Decisión de ejecución:** módulo gateado por permiso PROPIO `rendimiento` (el SQL otorga `rendimiento:write` a superadmin/admin en la tabla `roles`); RLS por `fn_role_can('finanzas')`. "Facturado" §9.5 = comprobantes emitidos al cliente (excluye anulada/error/rechazada) — confirmar con Fede si quiere exactamente `estado='emitida'`. **Restan en Fase 8 (deudas globales, NO bloquean el módulo):** (a) **fix global IVA** — `fn_asiento_auto_egreso` no desglosa IVA crédito fiscal (3ª línea DEBE `1.1.04.01` por `comprobantes_recibidos.iva` cuando el egreso tiene comprobante; afecta a TODO Finanzas → SQL aislado + test + avisar a Sofi; ver oneshot §6); (b) anular egreso no revierte el asiento (reversa manual); (c) **auditoría de integridad** Finanzas/Contab. (egreso→comprobante→asiento→saldos→libro — "hay cabos sueltos", pedido de Fede).
