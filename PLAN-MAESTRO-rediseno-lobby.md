...
- **Logística — REPENSAR** (Fede 2026-06-07): con los vehículos movidos a **Flota** (ACTIVOS), Logística cambia de rol → se nuclea en cargas/transporte conectado a Eventos y **consume** la Flota. **Transporte de Eventos pasa de `logistica_movimientos` a `cargas`** (ver AUDITORIA-2B). Redefinir su alcance al encarar Fase 4.
  ...
  - **❓ Preguntas abiertas para el remito v1 (a definir con Fede):** (a) ¿el remito se arma **por proyecto** o **por evento** (un remito con todos los stands)? ¿o ambos? (b) Los **items que van**: carga **manual** (descripción + cantidad) por ahora — ¿ok hasta que el cotizador escriba `cotizacion_items`? (c) **Firma**: ¿foto de la firma (subir imagen, como el remito firmado de hoy) o firma en pantalla (canvas)? (d) ¿la pestaña **Vehículos** sale de Logística (ya está Flota) o queda? (e) ¿el remito guarda historial (tabla `remitos`) o es solo generar PDF al vuelo?
