...
- **🆕 "Rendimiento por evento" → MÓDULO PROPIO · BLUEPRINT CERRADO (2026-06-18):** spec madre en **`docs/modulo-rendimiento-evento-blueprint.md`** (producido vía workflow multi-agente: 5 mappers verificaron prod + 3 críticos adversariales). Reemplaza el Excel de pagos de Lelean. Módulo `#rendimiento` (ADMIN Y FINANZAS, admin/superadmin) que **REUSA la plomería de Finanzas** (egresos/comprobantes/asientos/saldos), no la reimplementa. 3 tablas nuevas (`evento_costo_catalogo` + `evento_costos` + `evento_costo_pagos`) + `evento_rendimiento` (materiales manual). **Planilla** 5 categorías colapsables (catálogo configurable que reusa personas/proveedor + tarifa default · autocomplete con badge "editado") · **pagos SIEMPRE discriminados** (N egresos, nada de consolidado) con adelantos/tandas → comprobante recibido (IVA) en proveedores → cascada asiento automático. **Dashboard** Cobrado + Facturado lado a lado − costos − **materiales = insumos consumidos, NO la receta** (precio de alquiler). Features: presupuesto-vs-real · duplicar planilla · comparar eventos. Mockup visual `mockup-rendimiento-evento.html` (su "egreso consolidado" quedó OBSOLETO). **Etapas SQL-first REND.1→REND.5.** **RRHH.5 depende del contrato §7** (persona_id/fase/dias/tarifa/monto_pagado/egreso_id). **2 DEUDAS GLOBALES de Finanzas que el blueprint marca como gates:** (a) `fn_asiento_auto_egreso` NO desglosa IVA crédito fiscal → con proveedor facturado el balance no cuadra el crédito (fix global = 3ª línea DEBE `1.1.04.01`, coordinar Sofi); (b) anular egreso NO revierte el asiento (reversa manual). + Gate REND.1: `mapeo_cuentas` con las 5 categorías de egreso seedeadas o el asiento sale en silencio. **Pregunta abierta §9.5:** confirmar que "Facturado" = emitido al cliente (ingresos), no costos con factura recibida.
