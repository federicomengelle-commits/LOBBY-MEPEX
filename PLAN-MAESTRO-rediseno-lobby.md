# PLAN MAESTRO — Rediseño integral LOBBY-MEPEX  ·  RESTANTE ≈ 24%

> **Documento vivo = lo que FALTA hacer.** Lo que YA se hizo vive en `PROGRESO.md` (≈76%). No repetir acá lo que está en PROGRESO.
...
- **🆕 ESTADO INTEGRACIONES EXTERNAS — trámite ARRANCADO 2026-06-21** (sesión guiada con Fede sobre las consolas; sin código nuevo). Memorias `project_gmail_api_gcp_blocker` + `project_whatsapp_meta_coexistence`.
  ...
  - **ARCA — sin arrancar.** Facturación nativa (deprecar La PyME; greenfield → sin migración). Bloqueado por el trámite de Clave Fiscal / certificado X.509 de Fede. Vive en Fase 8 (ex Fase D del refactor de Finanzas).
