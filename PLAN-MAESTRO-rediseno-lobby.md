# PLAN MAESTRO — Rediseño integral LOBBY-MEPEX  ·  RESTANTE ≈ 65%
...
- **Taller** ⏳ EN CURSO (SB1+SB2 hechos — ver PROGRESO §Taller) = tablero de producción por proyecto con **checks EDITABLES** pre-poblados por plantilla (Estructura · Pintura · Gráfica · Equipamiento · Iluminación · Listo para cargar). El encargado tilda/edita; tildar desde la card, estado auto-sugerido, "más dinamismo" (pills/animaciones/urgencia). *(v2: derivar del BOM.)* Checklist unificado en `taller_proyecto_checklist` (el badge ya apunta ahí). **Falta SB3** (gatillo "Pasar a Taller" + filtro `en_taller`) **y SB4** (detalle del stand read-only + taller pierde `proyectos`).
